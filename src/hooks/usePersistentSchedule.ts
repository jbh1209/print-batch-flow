    ...
    try {
      ...
      scheduleData.forEach((entry: any) => {
        ...
        const job: DynamicScheduledJob = {
          id: entry.job_id,
          wo_no: entry.production_jobs.wo_no,
          customer: entry.production_jobs.customer,
          status: entry.production_jobs.status,
          ...
          due_date: new Date(entry.production_jobs.due_date),
          stage_order: 1,
