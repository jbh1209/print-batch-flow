...
export const BarcodeLabelsManager: React.FC<BarcodeLabelsManagerProps> = ({
  ...
  const generateLabels = async () => {
    if (selectedJobs.length === 0) {
      toast.error("No jobs selected for barcode label generation");
      return;
    }
    ...
    try {
      ...
      console.log("Function being called: downloadBarcodeLabelsPDF");
      
      const labelData: BarcodeLabelData[] = selectedJobs.map(job => ({
        id: job.id,
        wo_no: job.wo_no,
      ...
      console.log("About to call downloadBarcodeLabelsPDF...");
      
      // EXPLICITLY call the PDF generation function
      const result = await downloadBarcodeLabelsPDF(labelData, `barcode-labels-${selectedJobs.length}-jobs.pdf`);
      
      console.log("PDF generation result:", result);
      
      toast.success(`Successfully generated barcode labels PDF for ${selectedJobs.length} jobs`);
      handleClose();
    } catch (error) {
      console.error('Error generating barcode labels PDF:', error);
      toast.error('Failed to generate barcode labels PDF');
    } finally {
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span>Centered barcode (70mm)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  ...
                  <li>• Barcodes are in CODE128 format (widely supported)</li>
                </ul>
              </div>
            </div>
          </div>
