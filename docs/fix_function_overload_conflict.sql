-- ============================================================
-- FIX: Drop all overloaded versions and recreate with correct signature
-- ============================================================
-- Problem: Multiple overloaded versions of scheduler_reschedule_all_parallel_aware exist
-- Solution: Explicitly drop both versions, then create single correct version
-- ============================================================

-- Drop all overloaded versions explicitly
DROP FUNCTION IF EXISTS public.scheduler_reschedule_all_parallel_aware(timestamp with time zone) CASCADE;
DROP FUNCTION IF EXISTS public.scheduler_reschedule_all_parallel_aware(timestamp with time zone, boolean) CASCADE;

-- Now create the single correct version
CREATE OR REPLACE FUNCTION public.scheduler_reschedule_all_parallel_aware(
  p_start_from timestamptz DEFAULT NULL
) RETURNS jsonb
    LANGUAGE plpgsql
    SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
  v_base_start timestamptz;
  v_updated_count int := 0;
  v_violations jsonb := '[]'::jsonb;
  v_wrote_slots int := 0;
BEGIN
  RAISE NOTICE '🚀 scheduler_reschedule_all_parallel_aware: start_from=%', p_start_from;

  -- Phase 1: Clear existing schedule
  RAISE NOTICE '🗑️ Clearing existing schedule...';
  DELETE FROM public.stage_time_slots;
  UPDATE public.job_stage_instances 
  SET scheduled_start_at = NULL, scheduled_end_at = NULL;

  -- Determine base start time
  v_base_start := COALESCE(
    p_start_from,
    date_trunc('day', now() AT TIME ZONE 'Africa/Johannesburg') + interval '1 day' + interval '8 hours'
  ) AT TIME ZONE 'Africa/Johannesburg';
  RAISE NOTICE '📅 Base start time: %', v_base_start;

  -- Phase 2: Schedule stages respecting dependencies
  WITH RECURSIVE stage_schedule AS (
    -- Base case: stages with no dependencies
    SELECT
      jsi.id,
      jsi.job_id,
      jsi.stage_order,
      jsi.stage_name,
      jsi.estimated_duration_minutes,
      jsi.machine_type,
      v_base_start AS slot_start,
      v_base_start + (jsi.estimated_duration_minutes || ' minutes')::interval AS slot_end,
      0 AS depth
    FROM public.job_stage_instances jsi
    WHERE jsi.stage_order = 1
      AND jsi.stage_status != 'completed'

    UNION ALL

    -- Recursive case: stages with dependencies
    SELECT
      jsi.id,
      jsi.job_id,
      jsi.stage_order,
      jsi.stage_name,
      jsi.estimated_duration_minutes,
      jsi.machine_type,
      GREATEST(
        ss.slot_end,
        -- Check if a gap exists due to missing predecessor
        CASE 
          WHEN EXISTS (
            SELECT 1 FROM public.job_stage_instances pred
            WHERE pred.job_id = jsi.job_id
              AND pred.stage_order < jsi.stage_order
              AND pred.scheduled_start_at IS NULL
          )
          THEN ss.slot_end + interval '1 hour'  -- Add gap to flag precedence issue
          ELSE ss.slot_end
        END
      ) AS slot_start,
      GREATEST(
        ss.slot_end,
        CASE 
          WHEN EXISTS (
            SELECT 1 FROM public.job_stage_instances pred
            WHERE pred.job_id = jsi.job_id
              AND pred.stage_order < jsi.stage_order
              AND pred.scheduled_start_at IS NULL
          )
          THEN ss.slot_end + interval '1 hour'
          ELSE ss.slot_end
        END
      ) + (jsi.estimated_duration_minutes || ' minutes')::interval AS slot_end,
      ss.depth + 1 AS depth
    FROM public.job_stage_instances jsi
    JOIN stage_schedule ss 
      ON jsi.job_id = ss.job_id 
      AND jsi.stage_order = ss.stage_order + 1
    WHERE jsi.stage_status != 'completed'
  ),
  -- Pre-aggregate completed barriers to avoid nested aggregate error
  barrier_agg AS (
    SELECT 
      ss.id,
      jsonb_object_agg(
        dep.stage_name,
        jsonb_build_object(
          'completed_at', dep.completed_at,
          'max_completed', max_completed
        )
      ) AS completed_barriers_json
    FROM stage_schedule ss
    LEFT JOIN LATERAL (
      SELECT 
        pred.stage_name,
        pred.completed_at,
        MAX(pred.completed_at) OVER (PARTITION BY pred.job_id) as max_completed
      FROM public.job_stage_instances pred
      WHERE pred.job_id = ss.job_id
        AND pred.stage_order < ss.stage_order
        AND pred.stage_status = 'completed'
    ) dep ON true
    GROUP BY ss.id
  ),
  updated_stages AS (
    INSERT INTO public.stage_time_slots (
      production_stage_id,
      stage_instance_id,
      slot_start_time,
      slot_end_time,
      duration_minutes,
      job_id,
      date
    )
    SELECT
      ps.id,
      ss.id,
      ss.slot_start,
      ss.slot_end,
      ss.estimated_duration_minutes,
      ss.job_id,
      DATE(ss.slot_start AT TIME ZONE 'Africa/Johannesburg')
    FROM stage_schedule ss
    JOIN public.production_stages ps 
      ON LOWER(TRIM(ps.name)) = LOWER(TRIM(ss.stage_name))
    RETURNING stage_instance_id
  ),
  schedule_updates AS (
    UPDATE public.job_stage_instances jsi
    SET 
      scheduled_start_at = ss.slot_start,
      scheduled_end_at = ss.slot_end
    FROM stage_schedule ss
    WHERE jsi.id = ss.id
    RETURNING jsi.id, jsi.job_id, jsi.stage_order, jsi.stage_name, 
              ss.slot_start, ss.slot_end,
              EXTRACT(EPOCH FROM (ss.slot_start - v_base_start))/86400.0 AS days_saved
  )
  SELECT 
    COUNT(DISTINCT su.id)::int,
    COALESCE(
      jsonb_agg(
        DISTINCT jsonb_build_object(
          'job_id', su.job_id,
          'stage_order', su.stage_order,
          'stage_name', su.stage_name,
          'scheduled_start', su.slot_start,
          'days_saved', ROUND(su.days_saved::numeric, 2),
          'completed_barriers', ba.completed_barriers_json
        )
      ) FILTER (WHERE su.days_saved >= 0.25),  -- Only flag if 6+ hours saved
      '[]'::jsonb
    )
  INTO v_updated_count, v_violations
  FROM schedule_updates su
  LEFT JOIN barrier_agg ba ON ba.id = su.id;

  SELECT COUNT(*)::int INTO v_wrote_slots FROM public.stage_time_slots;

  RAISE NOTICE '✅ Scheduled % stages, wrote % slots, % precedence notes', 
    v_updated_count, v_wrote_slots, jsonb_array_length(v_violations);

  RETURN jsonb_build_object(
    'success', true,
    'scheduled_count', v_updated_count,
    'wrote_slots', v_wrote_slots,
    'violations', v_violations
  );
END;
$$;

ALTER FUNCTION public.scheduler_reschedule_all_parallel_aware(timestamptz) OWNER TO postgres;

-- ============================================================
-- VERIFICATION TEST
-- ============================================================
-- Test the function
SELECT public.scheduler_reschedule_all_parallel_aware(now()::timestamptz);

-- Expected: JSONB with keys: success, scheduled_count, wrote_slots, violations
-- No "function is not unique" error should occur
-- ============================================================
