-- Fix Phase 2 gap-filling: Add resource availability pre-check before calling find_available_gaps()
-- This ensures stages schedule at earliest possible time when resource is completely free
...
BEGIN
  ...
  -- CROSS-DIVISION: Initialize ALL resources for pending stages
  INSERT INTO _stage_tails(stage_id, next_available_time)
  ...
  -- ========== PHASE 2: ENHANCED GAP-FILLING WITH RESOURCE AVAILABILITY PRE-CHECK ==========
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling with Resource Availability Pre-Check';
  ...
  LOOP
    ...
    -- CRITICAL FIX: Pre-check if resource is free at earliest_possible_start
    -- This handles the common case where resource is completely free between predecessor end and FIFO start
    SELECT COUNT(*) INTO v_conflict_count
    FROM stage_time_slots
    WHERE production_stage_id = gap_candidate.production_stage_id
      AND slot_start_time < (earliest_possible_start + (gap_candidate.scheduled_minutes || ' minutes')::interval)
      AND slot_end_time > earliest_possible_start
      AND COALESCE(is_completed, false) = false;
    
    IF v_conflict_count = 0 AND earliest_possible_start < original_start THEN
      -- Resource is completely free! Schedule directly at earliest_possible_start
      new_start := earliest_possible_start;
      new_end := earliest_possible_start + (gap_candidate.scheduled_minutes || ' minutes')::interval;
      days_saved := EXTRACT(epoch FROM (original_start - new_start)) / 86400.0;
      hours_saved := days_saved * 24;
      
      RAISE NOTICE '🎯 Direct scheduling: % can start at earliest_possible (resource free), saves %.1f days', 
      ...
      INSERT INTO schedule_gap_fills(
        ...
        original_start, new_start, days_saved, (hours_saved * 60)::integer, 'reschedule_all_direct'
      );
      
      gap_filled_count := gap_filled_count + 1;
    ELSE
      -- Resource has conflicts, fall back to find_available_gaps() for optimization between bookings
      ...
      RAISE NOTICE '🔍 Gap-fill candidate (with conflicts): % (stage %), lookback: % days (gap from prev: %.1f days)', 
      ...
      IF best_gap IS NOT NULL AND days_saved >= 0.25 THEN
        ...
        INSERT INTO schedule_gap_fills(
          ...
          gap_candidate.job_id, gap_candidate.stage_instance_id, gap_candidate.production_stage_id,
          original_start, new_start, days_saved, (hours_saved * 60)::integer, 'reschedule_all_gap_search'
        );
...
COMMENT ON FUNCTION public.scheduler_reschedule_all_by_division(text, timestamp with time zone) IS 
'Division-aware scheduler with FIXED Phase 2 gap-filling: Pre-checks if resource is free at earliest_possible_start before searching for gaps between bookings. This prevents wasted working days when resources are completely free.';
