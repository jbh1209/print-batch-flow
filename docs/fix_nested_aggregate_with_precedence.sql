-- ============================================================
-- FIX: Nested aggregate error in completed_barriers + keep precedence check
-- ============================================================
-- Problem: Lines 82-96 use MAX() inside jsonb_object_agg() causing error 42803
-- Solution: Pre-aggregate MAX values in subquery, then apply jsonb_object_agg
-- Also keeps: precedence check, no 120-min cap, 6-hour threshold
-- ============================================================
...
BEGIN
  ...
  -- FIX: Pre-aggregate MAX values before jsonb_object_agg
  completed_barriers := (
    SELECT COALESCE(
      jsonb_object_agg(
        job_id_text,
    ...
    FROM (
      SELECT
        jsi.job_id::text AS job_id_text,
        COALESCE(MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment = 'both'), base_time) AS main_time,
        COALESCE(MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment IN ('cover', 'both')), base_time) AS cover_time,
        COALESCE(MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment IN ('text', 'both')), base_time) AS text_time,
        COALESCE(MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment = 'both'), base_time) AS both_time
  ...
  LOOP
    ...
    LOOP
      ...
      SELECT GREATEST(
        ...
        (job_stage_barriers ->> 'both')::timestamptz
      )
      INTO max_barrier_time;
      
      job_stage_barriers := jsonb_build_object(
        'main', max_barrier_time,
        'cover', max_barrier_time,
        'text', max_barrier_time,
        'both', max_barrier_time
      );
    END LOOP;
  END LOOP;

  -- PHASE 2: GAP-FILLING (NO CAP, 6-hour minimum threshold, with precedence check)
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling (no cap, 6h min threshold, precedence enforced)';
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL AND best_gap.days_earlier >= 0.25 THEN
      ...
      -- PRECEDENCE CHECK: Ensure gap-fill respects sequential stage order
      ...
      -- If moving to the gap would violate stage order, skip this gap-fill
      IF predecessor_end IS NOT NULL AND best_gap.gap_start < predecessor_end THEN
        RAISE NOTICE '⏭️ GAP-FILL SKIPPED: Stage % (WO: %, order %) would start at % but predecessor ends at %',
          gap_candidate.stage_name, gap_candidate.wo_no, gap_candidate.stage_order, best_gap.gap_start, predecessor_end;
        CONTINUE;
      END IF;
      
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % (WO: %) from % to % (saves %.1f days)',
  ...
  RAISE NOTICE 'Parallel-aware reschedule complete: % slots written, % stages updated, % gap-filled, % expired, % on-hold',
...
-- ============================================================
-- Run this to test:
-- SELECT * FROM public.scheduler_reschedule_all_parallel_aware(now()::timestamptz);
-- ============================================================
