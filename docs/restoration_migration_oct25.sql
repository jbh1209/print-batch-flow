-- ============================================================================
-- OCT 25TH DATABASE RESTORATION SQL
-- ============================================================================
-- 
-- This script restores the exact working state of the scheduler functions
-- from the Oct 25th SQL dump backup. These functions were verified to work
-- correctly in production with no 22P02 errors.
--
-- EXECUTION INSTRUCTIONS:
-- 1. Open Supabase SQL Editor: https://supabase.com/dashboard/project/kgizusgqexmlfcqfjopk/sql/new
-- 2. Copy this entire file contents
-- 3. Paste into the SQL Editor
-- 4. Execute the script
-- 5. Verify success by testing a reschedule from your UI
--
-- WHAT THIS SCRIPT DOES:
-- - Phase 1: Drops all broken/recent scheduler function variants
-- - Phase 2: Restores three critical Oct 25th functions:
--   1. simple_scheduler_wrapper (entry point for edge function)
--   2. scheduler_reschedule_all_parallel_aware (full FIFO + gap-fill reschedule)
--   3. scheduler_append_jobs (append-only scheduling for new jobs)
--
-- NO MODIFICATIONS - This is a verbatim copy from the SQL dump
-- ============================================================================

-- ============================================================================
-- PHASE 1: DROP BROKEN FUNCTIONS
-- ============================================================================

-- Drop the broken resource-fill function and any overloads
DROP FUNCTION IF EXISTS public.scheduler_resource_fill_optimized() CASCADE;
DROP FUNCTION IF EXISTS public.scheduler_resource_fill_optimized(uuid[]) CASCADE;
DROP FUNCTION IF EXISTS public.scheduler_resource_fill_optimized(timestamptz) CASCADE;

-- Drop any other post-Oct 25th scheduler variants that may cause conflicts
DROP FUNCTION IF EXISTS public.scheduler_append_jobs(uuid[], boolean, uuid) CASCADE;

-- Drop the functions we're about to restore to ensure clean creation
...
-- ============================================================================
-- PHASE 2: RESTORE OCT 25TH WORKING FUNCTIONS
-- ============================================================================

-- ----------------------------------------------------------------------------
-- FUNCTION 1: scheduler_append_jobs
-- Lines 8476-8985 from SQL dump
-- Atomic per-job scheduling with part-aware barriers and gap-filling
-- ----------------------------------------------------------------------------

CREATE FUNCTION public.scheduler_append_jobs(p_job_ids uuid[], p_only_if_unset boolean DEFAULT true) RETURNS TABLE(wrote_slots integer, updated_jsi integer, violations jsonb)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
-- ----------------------------------------------------------------------------
-- FUNCTION 2: scheduler_reschedule_all_parallel_aware
-- Lines 9236-9715 from SQL dump
-- Full FIFO scheduling with parallel barriers and gap-filling
-- ----------------------------------------------------------------------------

CREATE FUNCTION public.scheduler_reschedule_all_parallel_aware(p_start_from timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS TABLE(wrote_slots integer, updated_jsi integer, violations jsonb)
    LANGUAGE plpgsql
    AS $$
DECLARE
  base_time timestamptz;
...
-- ----------------------------------------------------------------------------
-- FUNCTION 3: simple_scheduler_wrapper
-- Lines 11344-11372 from SQL dump
-- Entry point wrapper for edge function calls
-- ----------------------------------------------------------------------------

CREATE FUNCTION public.simple_scheduler_wrapper(p_mode text DEFAULT 'reschedule_all'::text, p_start_from timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS jsonb
...
-- ============================================================================
--
-- Three functions have been restored to their Oct 25th working state:
-- 1. scheduler_append_jobs - Atomic per-job scheduling with rollback
-- 2. scheduler_reschedule_all_parallel_aware - Full FIFO + gap-fill scheduler  
-- 3. simple_scheduler_wrapper - Edge function entry point
--
-- Next steps:
-- 1. Test reschedule from UI (Generate Schedule button)
-- 2. Check Supabase Edge Function logs for success messages
-- 3. Verify slots are written to stage_time_slots table
-- 4. Test proof approval flow (automatic append)
--
-- Expected result: No 22P02 errors, clean execution with proper JSONB handling
-- ============================================================================
