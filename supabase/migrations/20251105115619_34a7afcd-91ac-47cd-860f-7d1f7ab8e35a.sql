-- Fix scheduler_resource_fill_optimized to track resource availability per part assignment
-- This enables parallel processing: Cover and Text parts don't block each other on shared resources
...
BEGIN
  ...
  resource_avail AS (
    ...
    FROM stage_queue sq
    LEFT JOIN stage_time_slots sts 
      ON sts.production_stage_id = sq.production_stage_id
    LEFT JOIN job_stage_instances jsi_slot 
      ON jsi_slot.id = sts.stage_instance_id
    WHERE sts.stage_instance_id IS NULL
      OR (
        -- Cover stages wait for Cover and Both slots only
        (sq.part_assignment = 'cover' AND jsi_slot.part_assignment IN ('cover', 'both'))
        -- Text stages wait for Text and Both slots only
        OR (sq.part_assignment = 'text' AND jsi_slot.part_assignment IN ('text', 'both'))
        -- Both stages wait for all slots
        OR (sq.part_assignment = 'both')
      )
    GROUP BY sq.production_stage_id, sq.part_assignment
  )
  ...
  FROM stage_queue sq
  JOIN resource_avail ra 
    ON ra.production_stage_id = sq.production_stage_id
    AND ra.part_assignment = sq.part_assignment;
