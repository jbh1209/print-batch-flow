-- Fix scheduler dependency violations by restructuring to job-by-job processing
...
DECLARE
  ...
  -- Job and stage processing variables
  r_job record;
  r_stage record;
  ...
  job_part_barrier_time timestamptz;
  ...
  -- Compatible counter for number of pre-seeded jobs (older Postgres without jsonb_object_length)
  job_part_barriers_count integer := 0;
BEGIN
  ...
  RAISE NOTICE 'JOB-SEQUENTIAL Scheduler V3: Starting from %', base_time;
  ...
  RAISE NOTICE 'JOB-SEQUENTIAL Scheduler V3: Cleared % slots and % instances', clear_result.cleared_slots, clear_result.cleared_instances;
  ...
  RAISE NOTICE 'JOB-SEQUENTIAL Scheduler V3: Initialized % production stages', (SELECT COUNT(*) FROM _stage_tails);
  ...
  -- Compatible count of jobs in job_part_barriers
  SELECT COUNT(*) INTO job_part_barriers_count
  FROM jsonb_object_keys(job_part_barriers);
  job_part_barriers_count := COALESCE(job_part_barriers_count, 0);

  RAISE NOTICE 'JOB-SEQUENTIAL V3: Pre-seeded part barriers for % jobs', job_part_barriers_count;

  -- ARCHITECTURAL FIX: Process jobs one by one, then stages within each job sequentially
  FOR r_job IN
    SELECT 
      pj.id as job_id,
    ...
    WHERE COALESCE(jsi.status, '') NOT IN ('completed', 'active')
      AND pj.proof_approved_at IS NOT NULL  -- Only process proof-approved jobs
    GROUP BY pj.id, pj.proof_approved_at, pj.wo_no
    ORDER BY pj.proof_approved_at ASC, pj.id ASC  -- FIFO job ordering
  LOOP
    RAISE NOTICE 'JOB-SEQUENTIAL V3: Processing job % (WO: %) with % stages', 
      r_job.job_id, r_job.wo_no, r_job.total_stages;
    
    -- CRITICAL FIX: Initialize job barriers if not already present
    IF NOT (job_part_barriers ? r_job.job_id::text) THEN
      job_part_barriers := jsonb_set(
        job_part_barriers, 
        ARRAY[r_job.job_id::text], 
        jsonb_build_object(
          'cover', GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time)),
          'text', GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time)),
          'both', GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time))
        )
      );
    END IF;
    
    -- Process all stages within this job sequentially by stage_order
    FOR r_stage IN
      SELECT 
      ...
      ORDER BY 
        COALESCE(jsi.stage_order, 999999) ASC,  -- Sequential stage processing within job
        jsi.id ASC  -- Deterministic tie-breaker
    LOOP
      -- Calculate part-aware job barrier time
      CASE 
        WHEN r_stage.part_assignment = 'cover' THEN
          job_part_barrier_time := COALESCE(
            (job_part_barriers -> r_job.job_id::text ->> 'cover')::timestamptz, 
            GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time))
          );
        WHEN r_stage.part_assignment = 'text' THEN
          job_part_barrier_time := COALESCE(
            (job_part_barriers -> r_job.job_id::text ->> 'text')::timestamptz, 
            GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time))
          );
        WHEN r_stage.part_assignment = 'both' THEN
          -- 'Both' stages wait for ALL prerequisite parts (convergence point)
          job_part_barrier_time := GREATEST(
            COALESCE((job_part_barriers -> r_job.job_id::text ->> 'cover')::timestamptz, base_time),
            COALESCE((job_part_barriers -> r_job.job_id::text ->> 'text')::timestamptz, base_time),
            GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time))
          );
        ELSE
          -- Default stages use general job barrier
          job_part_barrier_time := GREATEST(base_time, COALESCE(r_job.proof_approved_at, base_time));
      END CASE;
      ...
      -- CRITICAL: Stage MUST wait for BOTH part dependency AND resource availability
      stage_earliest_start := GREATEST(job_part_barrier_time, resource_available_time);

      RAISE NOTICE 'JOB-SEQUENTIAL V3: Scheduling %: job=% part=% order=% duration=%min, earliest_start=%, job_barrier=%, resource_avail=%',
        r_stage.stage_name, r_job.wo_no, COALESCE(r_stage.part_assignment, 'default'), r_stage.stage_order, r_stage.duration_minutes, 
        stage_earliest_start, job_part_barrier_time, resource_available_time;
      ...
      -- CRITICAL FIX: Update part-specific barriers IMMEDIATELY for proper dependencies
      IF r_stage.part_assignment IN ('cover', 'text', 'both') THEN
        ...
        RAISE NOTICE 'JOB-SEQUENTIAL V3: Updated %s barrier to %s for job %s (affects remaining stages in this job)',
          r_stage.part_assignment, actual_end_time, r_stage.job_id;
      END IF;

      RAISE NOTICE 'JOB-SEQUENTIAL V3: Completed stage % - ACTUAL times: %s to %s',
        r_stage.stage_name, actual_start_time, actual_end_time;
    END LOOP;
    
    RAISE NOTICE 'JOB-SEQUENTIAL V3: Completed job % (WO: %)', r_job.job_id, r_job.wo_no;
  END LOOP;
  ...
  RAISE NOTICE 'JOB-SEQUENTIAL Scheduler V3 COMPLETE: % slots written, % instances updated, % synced, % violations remain', 
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'JOB-SEQUENTIAL Scheduler V3 failed: %', SQLERRM;
END;
$function$;