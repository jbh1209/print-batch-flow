-- Refactor scheduler_append_jobs to use job-first scheduling (matching Oct 24 behavior)
-- This replaces the stage-wave approach with FIFO job processing
...
BEGIN
  ...
  RAISE NOTICE '🎯 JOB-FIRST scheduler starting from: %', base_time;
  ...
  -- Build completed barriers per job
  completed_barriers := (
  ...
  -- PHASE 1: JOB-FIRST SCHEDULING
  RAISE NOTICE '🎯 Phase 1: JOB-FIRST Scheduling for % jobs', array_length(p_job_ids, 1);
  ...
  LOOP
    ...
    RAISE NOTICE '➡️ Scheduling job % (WO: %) from base %', 
      r_job.job_id, r_job.wo_no, r_job.proof_approved_at;
    
    -- Process ALL stages for this job in stage_order
    FOR r_stage IN
      SELECT 
    ...
    LOOP
      ...
      -- Calculate barrier-based earliest start ('both' depends only on cover+text barriers)
      IF r_stage.part_assignment = 'both' THEN
        ...
        RAISE NOTICE '  📌 Stage % (order %, part=both): cover_barrier=%, text_barrier=%, earliest=%',
          r_stage.stage_name, r_stage.stage_order, cover_barrier_time, text_barrier_time, stage_earliest_start;
      ELSE
        ...
        RAISE NOTICE '  📌 Stage % (order %, part=%): barrier=%',
          r_stage.stage_name, r_stage.stage_order, barrier_key, stage_earliest_start;
      END IF;
      ...
      IF resource_available_time > stage_earliest_start THEN
        RAISE NOTICE '  🔧 Resource tail delayed start from % to %', stage_earliest_start, resource_available_time;
      END IF;
      
      stage_earliest_start := GREATEST(stage_earliest_start, resource_available_time);

      -- Enforce predecessor dependencies with part-aware filtering
      ...
      IF predecessor_end IS NOT NULL AND predecessor_end > stage_earliest_start THEN
        RAISE NOTICE '  🔒 Predecessor delayed start from % to %', stage_earliest_start, predecessor_end;
      ...
      RAISE NOTICE '  ✅ Job % (WO: %) stage % (part=%, order %): % → %',
        r_job.job_id, r_job.wo_no, r_stage.stage_name, 
        COALESCE(r_stage.part_assignment, 'main'), r_stage.stage_order,
    ...
    ELSE
      -- Store barriers for potential future use
      ...
      RAISE NOTICE '✅ Job % (WO: %) complete', r_job.job_id, r_job.wo_no;
    END IF;
  END LOOP;
...
COMMENT ON FUNCTION public.scheduler_append_jobs IS 
'Job-first scheduler: processes jobs in FIFO order (by proof_approved_at), scheduling all stages per job before moving to next job. Includes gap-filling phase for eligible stages.';