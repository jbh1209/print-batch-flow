-- Fix gap-filler threshold: accept gaps between predecessor and FIFO time
-- This replaces the strict >= 0.25 days threshold with a more logical check
...
BEGIN
  ...
  LOOP
    ...
    -- FIXED THRESHOLD: Accept gap if it exists, is after predecessor, and is before FIFO time
    -- Calculate explicit days saved compared to FIFO scheduled time
    IF best_gap IS NOT NULL THEN
      days_saved := EXTRACT(EPOCH FROM (original_start - best_gap.gap_start)) / 86400.0;
      
      -- Accept gap if it's between predecessor end and FIFO scheduled time
