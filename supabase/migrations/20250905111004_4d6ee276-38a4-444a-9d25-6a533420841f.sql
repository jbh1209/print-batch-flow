...
BEGIN
  ...
  LOOP
    ...
    current_time := r_stage.next_available_time;
    ...
    FOR r_stage IN
      ...
      WHERE jsi.production_stage_id IN (
        -- Only consider resources that are available at current_time
        SELECT stage_id FROM _stage_tails WHERE next_available_time <= current_time + interval '1 minute'
      )
      ...
      AND current_time >= COALESCE((job_barriers ->> jsi.job_id::text)::timestamptz, base_time)
    ...
    LOOP
      ...
      SELECT next_available_time INTO current_time
      FROM _stage_tails 
      ...
      current_time := GREATEST(current_time, job_barrier);
      ...
      RAISE NOTICE 'Scheduling stage % (%): % mins from % (resource avail: %, job barrier: %)',
        ...
        current_time, (SELECT next_available_time FROM _stage_tails WHERE stage_id = r_stage.production_stage_id),
        job_barrier;

      -- Place the duration
      SELECT * INTO placement_result
      FROM public.place_duration_sql(current_time, r_stage.duration_minutes);
