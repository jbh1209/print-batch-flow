...
SET estimated_duration_minutes = (
  SELECT 
    CASE 
      ...
      WHEN jsi.quantity IS NULL OR jsi.quantity = 0 THEN COALESCE(ps.make_ready_time_minutes, 10) -- Just setup time
      ELSE (
        CASE ps.speed_unit
          WHEN 'sheets_per_hour' THEN 
            CEIL((jsi.quantity::NUMERIC / ps.running_speed_per_hour::NUMERIC) * 60) + COALESCE(ps.make_ready_time_minutes, 10)
          WHEN 'minutes_per_item' THEN 
            (jsi.quantity * ps.running_speed_per_hour) + COALESCE(ps.make_ready_time_minutes, 10)
          ELSE 
            CEIL((jsi.quantity::NUMERIC / ps.running_speed_per_hour::NUMERIC) * 60) + COALESCE(ps.make_ready_time_minutes, 10)
        END
      )
