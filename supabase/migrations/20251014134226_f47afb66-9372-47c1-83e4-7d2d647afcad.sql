-- CRITICAL RESTORATION: Copy exact working Phase 1 + add Phase 2 gap-filling
-- 
-- STRICT CODING ENGINE INSTRUCTIONS:
-- 1. You MUST copy the Phase 1 code EXACTLY as written below - NO MODIFICATIONS
-- 2. You MUST preserve ALL variable declarations, ALL RAISE NOTICE statements, ALL logic
-- 3. You MUST use EXIT (not CONTINUE) on failures in Phase 1
-- 4. You MUST track atomicity with job_inserted_slot_ids and job_updated_stage_ids
-- 5. You MUST initialize resource tails from "WHERE slot_end_time >= base_time" (ALL future slots)
-- 6. You MUST use "RETURNING id INTO" pattern for inserted slots
-- 7. You MUST include complete rollback logic if job_failed = true
-- 8. After Phase 1 completes, you MUST add Phase 2 gap-filling (120-minute cap for append)
-- 9. You MUST NOT change function signature or return type
-- 10. You MUST NOT optimize, refactor, or "improve" the code - COPY IT EXACTLY
...
BEGIN
  -- Base time: use tomorrow's working start for append operations
  base_time := public.next_working_start(date_trunc('day', now()) + interval '1 day');
  RAISE NOTICE '🔄 Append jobs starting from: %', base_time;

  -- Only clean up for jobs being appended if p_only_if_unset is false
  ...
  -- CRITICAL: Initialize resource tails with ALL future slots (not just completed)
  INSERT INTO _stage_tails(stage_id, next_available_time)
  SELECT 
    production_stage_id, 
    COALESCE(MAX(slot_end_time), base_time)
  FROM stage_time_slots 
  WHERE slot_end_time >= base_time  -- ALL future slots
  ...
  LOOP
    -- CRITICAL: Reset atomicity tracking for each job
    ...
    LOOP
      FOR r_stage IN
        ...
        ORDER BY jsi.id
      LOOP
        -- CRITICAL: Use EXIT on failures, not CONTINUE
        ...
        -- CRITICAL: Enforce sequential stage order
        ...
        IF predecessor_end IS NOT NULL AND predecessor_end > stage_earliest_start THEN
          RAISE NOTICE '🔒 Job % (WO: %) stage % (order %): waiting for predecessor. New earliest: %',
            r_job.job_id, r_job.wo_no, r_stage.stage_name, r_stage.stage_order, predecessor_end;
        ...
        -- CRITICAL: Use RETURNING id INTO pattern
        ...
        -- Exit if we hit a conflict
        IF job_failed THEN
          EXIT;
        END IF;

        -- CRITICAL: Derive actual times from inserted rows
    ...
    -- CRITICAL: Complete atomicity rollback block
    IF job_failed THEN
  ...
  -- ============================================================================
  -- PHASE 2: GAP-FILLING (APPEND-SPECIFIC: 120-MINUTE CAP)
  -- ============================================================================
