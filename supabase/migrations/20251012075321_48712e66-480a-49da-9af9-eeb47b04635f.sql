-- Replace find_available_gaps with intra-day aware implementation
...
BEGIN
  -- Earliest we consider is today to avoid past days
  v_earliest_allowed_date := CURRENT_DATE;

  -- Scan window: honor lookback, today, and precedence alignment date
  ...
  -- End the scan the day after the FIFO target to allow same-day earlier slots
  ...
  LOOP
    -- Anchor to alignment (predecessor completion) and within the day window
    cur_start := GREATEST(day_win.win_start, COALESCE(p_align_at, day_win.win_start));

    -- Iterate existing non-completed bookings intersecting this window, ordered by start
    FOR s IN
      SELECT 
    ...
    LOOP
      ...
      -- Advance past this booking
      IF s.s_end > cur_start THEN
        cur_start := s.s_end;
      END IF;

      -- If we reached the end of this day window, move to next day
