-- Phase 3: Add validation guards to scheduler to prevent crashes from invalid durations
-- This updates the scheduler to validate duration before calling place_duration_sql
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      FOR r_stage IN
        ...
        ORDER BY jsi.id
      LOOP
        -- PHASE 3 VALIDATION: Check if duration is valid before attempting placement
        IF r_stage.duration_minutes IS NULL OR r_stage.duration_minutes <= 0 THEN
          ...
          -- Log this as a validation issue but don't crash the scheduler
          ...
          -- Skip this stage but continue with the rest
          CONTINUE;
        END IF;
  ...
  -- Gap-filling phase (unchanged, keeping existing logic)
  ...
  LOOP
    ...
    v_stage_move_cap := CASE WHEN v_is_finishing_stage THEN 30 ELSE 21 END;
    v_days_until_original := EXTRACT(epoch FROM (original_start - now())) / 86400.0;
    v_lookback_days := CASE 
      WHEN v_is_finishing_stage THEN LEAST(30, GREATEST(7, FLOOR(v_days_until_original)::integer))
      ELSE LEAST(30, GREATEST(7, FLOOR(v_days_until_original * 0.7)::integer))
    END;
    
    SELECT g.* INTO best_gap
    FROM find_available_gaps(
      gap_candidate.production_stage_id, gap_candidate.scheduled_minutes,
      original_start, v_lookback_days, earliest_possible_start
    ) g
    ...
    IF best_gap IS NOT NULL AND days_saved >= 0.25 AND days_saved <= v_stage_move_cap THEN
