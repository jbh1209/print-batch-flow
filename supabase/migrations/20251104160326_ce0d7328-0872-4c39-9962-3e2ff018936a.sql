-- Emergency fix: Add WHERE clause to nuclear wipe DELETE statement
-- This satisfies Supabase's safety requirement while still deleting all rows
-- Fixed: Added missing r_dep record declaration
...
DECLARE
  ...
  r_job record;
  r_stage record;
  r_dep record;  -- Added: missing declaration for dependency loop
  slot_record jsonb;
...
BEGIN
  ...
  RAISE NOTICE '📸 Captured % completed barriers before wipe', jsonb_object_length(COALESCE(barriers_snapshot, '{}'::jsonb));
    ...
    LOOP
      ...
      IF p_commit THEN
        ...
        SELECT
          r_stage.production_stage_id,
          r_stage.stage_instance_id,
          (slot ->> 'start')::timestamp with time zone,
          (slot ->> 'end')::timestamp with time zone,
          ((slot ->> 'start')::timestamp with time zone)::date
        FROM jsonb_array_elements(slot_record -> 'slots') AS slot;

        slots_written := slots_written + (jsonb_array_length(slot_record -> 'slots'));
      END IF;
    END LOOP;
  END LOOP;

  RAISE NOTICE '✅ Scheduler complete: updated_jsi=%, wore_slots=%', jsi_updated, slots_written;
