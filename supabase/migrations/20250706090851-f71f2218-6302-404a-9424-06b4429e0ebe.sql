-- Fix master queue action targeting by returning both actual stage ID and display stage ID
...
RETURNS TABLE(
  ...
  is_subsidiary_stage boolean, master_queue_stage_id uuid, display_stage_id uuid
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = p_user_id AND role = 'admin') THEN
    ...
    SELECT 
      ...
      -- FIXED: Keep actual stage ID for actions
      COALESCE(jcs.current_stage_id, '00000000-0000-0000-0000-000000000000'::uuid)::uuid as current_stage_id,
      ...
      -- NEW: Display stage ID for UI consolidation (master queue ID if subsidiary, otherwise actual stage ID)
      COALESCE(ps.master_queue_id, jcs.current_stage_id, '00000000-0000-0000-0000-000000000000'::uuid)::uuid as display_stage_id
  ...
  ELSE
    ...
    SELECT 
      ...
      COALESCE(esp.can_edit, false)::boolean as user_can_work,
