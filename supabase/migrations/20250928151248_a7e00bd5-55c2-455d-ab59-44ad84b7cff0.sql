-- Fix parallel-aware scheduler ordering logic to prevent cross-branch dependencies
...
BEGIN
  ...
  LOOP
    ...
    -- Store this job's barrier and initialize part barriers to job_current_barrier
    ...
    -- CRITICAL FIX: Process stages with PART-AWARE ordering to prevent cross-branch dependencies
    ...
    FOR r_stage IN
      ...
      ORDER BY 
        -- FIXED: Part-aware ordering to prevent cross-branch dependencies
        CASE WHEN jsi.part_assignment = 'both' THEN 999 ELSE COALESCE(jsi.stage_order, 999999) END ASC,
        -- Within same logical order, process specific parts before convergence ('both')
        CASE 
          WHEN jsi.part_assignment = 'both' THEN 3
          WHEN jsi.part_assignment = 'cover' THEN 1
          WHEN jsi.part_assignment = 'text' THEN 2
          ELSE 1
        END,
        jsi.id ASC
    LOOP
      -- LAYER-BASED PROCESSING: Check if we're starting a new layer
      -- Use logical stage order (not part-specific) for layer boundaries
      DECLARE
        logical_layer_order integer;
      BEGIN
        logical_layer_order := CASE 
          WHEN r_stage.part_assignment = 'both' THEN 999 
          ELSE COALESCE(r_stage.stage_order, 999999) 
        END;
        
        IF logical_layer_order != current_layer_order THEN
          ...
          -- Start new layer
          current_layer_order := logical_layer_order;
          layer_end_times := ARRAY[]::timestamptz[];
      ...
      IF r_stage.part_assignment = 'both' THEN
        ...
        BEGIN
          -- CRITICAL FIX: Get the CURRENT part completion times for this job from job_part_barriers
          job_part_data := job_part_barriers -> r_job.job_id::text;
          
          -- CRITICAL FIX: Use the ACTUAL part barriers (not job_current_barrier as fallback)
