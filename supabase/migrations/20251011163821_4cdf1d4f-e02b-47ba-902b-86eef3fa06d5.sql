-- Fix gap-filling to respect sequential stage order constraints
-- This prevents stages from being moved earlier if predecessor stages haven't completed
...
BEGIN
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL AND best_gap.days_earlier >= 0.25 THEN
      ...
      -- VALIDATION: Ensure gap-fill respects sequential stage order
      SELECT MAX(jsi2.scheduled_end_at) INTO predecessor_end
      FROM job_stage_instances jsi2
      WHERE jsi2.job_id = gap_candidate.job_id
        AND jsi2.stage_order < gap_candidate.stage_order
      ...
      IF predecessor_end IS NOT NULL AND best_gap.gap_start < predecessor_end THEN
        ...
        CONTINUE; -- Skip to next gap candidate
      END IF;
      ...
      IF placement_result.placement_success THEN
        ...
        -- Log the gap fill with CORRECT column names from working migration
