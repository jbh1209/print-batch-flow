-- Fix broken references in scheduler_append_jobs by removing non-existent columns and joining production_stages for stage names
-- Also implement a minimal, safe scheduling pass that writes scheduled_start_at/end_at on job_stage_instances only

-- 1) Drop old/broken function (if exists)
DROP FUNCTION IF EXISTS public.scheduler_append_jobs(uuid[], boolean);

-- 2) Recreate with correct references and contract
...
DECLARE
  v_updated integer := 0;
  v_wrote_slots integer := 0; -- This minimal version does not write to stage_time_slots
  job_rec record;
  stage_rec record;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Compute baseline from proof approvals, default now()
  ...
  -- Return counts compatible with edge function expectations
  RETURN QUERY SELECT v_wrote_slots, v_updated;
END;
$$;

-- Optional: grant execute to authenticated/service roles if needed
-- GRANT EXECUTE ON FUNCTION public.scheduler_append_jobs(uuid[], boolean) TO anon, authenticated, service_role;