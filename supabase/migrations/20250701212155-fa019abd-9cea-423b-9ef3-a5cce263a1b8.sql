-- Phase 2: Update job access function to handle conditional stages
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = p_user_id AND role = 'admin') THEN
      ...
      SELECT 
        ...
        CASE 
          WHEN ps.name = 'Batch Allocation' THEN 
            -- Show batch allocation only if job is not batch ready and has no batch references
          ...
          ELSE true -- Other conditional stages show by default
        END as should_show
    ...
    ORDER BY pj.wo_no;
  ELSE
    -- Non-admin users: use existing permission logic but add conditional stage support
    RETURN QUERY
    WITH user_groups AS (
      SELECT ugm.group_id
