-- Fix 1: Prevent scheduling in the past in find_available_gaps()
-- Fix 2: Add job-level dependency checks to gap-filling
-- Fix 3: Add business constraints to gap-filling

-- Step 1: Fix find_available_gaps() to never return gaps in the past
CREATE OR REPLACE FUNCTION public.find_available_gaps(
  p_stage_id uuid, 
  p_duration_minutes integer, 
  p_fifo_start_time timestamp with time zone, 
  p_lookback_days integer DEFAULT 7  -- REDUCED from 21 to 7
)
RETURNS TABLE(
...
-- Step 2: Fix scheduler_reschedule_all_parallel_aware() gap-filling logic
...
DECLARE
  ...
  -- NEW: Job-level dependency tracking for gap-filling
  earliest_possible_start timestamptz;
  v_min_future_threshold timestamptz;
  v_max_days_moveable constant integer := 5;  -- Max 5 days earlier
BEGIN
  ...
  -- ========== PHASE 2: GAP-FILLING OPTIMIZATION PASS (WITH CONSTRAINTS) ==========
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling Optimization Pass WITH BUSINESS CONSTRAINTS';
  ...
  FOR gap_candidate IN
    ...
    WHERE jsi.schedule_status = 'scheduled'
      ...
      -- NEW CONSTRAINT: Only gap-fill stages at least 2 days in the future
  ...
  LOOP
    ...
    -- NEW: Get earliest possible start time based on predecessor stages in the same job
    ...
    -- Ensure earliest_possible_start is not before now
    earliest_possible_start := GREATEST(earliest_possible_start, now());
    
    RAISE NOTICE '🔀 Considering gap-fill for stage % (order %): original_start=%, earliest_possible=%, lookback=7 days',
      gap_candidate.stage_name,
      gap_candidate.stage_order,
      original_start,
      earliest_possible_start;
    
    -- Find best gap (earliest available) with reduced lookback (7 days instead of 21)
    SELECT * INTO best_gap
    FROM find_available_gaps(
      gap_candidate.production_stage_id,
      gap_candidate.scheduled_minutes,
      original_start,
      7  -- REDUCED lookback from 21 to 7 days
    )
    WHERE gap_start >= earliest_possible_start  -- NEW: Respect job-level dependencies
    ORDER BY gap_start ASC
    LIMIT 1;
    
    -- NEW CONSTRAINT: Only move if found a gap AND it saves ≥1 day AND ≤5 days earlier
    IF best_gap IS NOT NULL 
       AND best_gap.days_earlier >= 1.0 
       AND best_gap.days_earlier <= v_max_days_moveable THEN
      
      days_saved := best_gap.days_earlier;
      
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % from % to % (saves %.1f days, respects dependencies)',
        gap_candidate.stage_name,
        original_start,
        best_gap.gap_start,
        days_saved;
    ...
    ELSIF best_gap IS NOT NULL AND best_gap.days_earlier > v_max_days_moveable THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage % - would move %.1f days (exceeds max %)',
        gap_candidate.stage_name, best_gap.days_earlier, v_max_days_moveable;
    END IF;
  END LOOP;
  
  RAISE NOTICE '✅ Gap-filling complete with constraints: % stages moved (max %d earlier, 7d lookback, 2d future threshold)',
    gap_filled_count, v_max_days_moveable;
