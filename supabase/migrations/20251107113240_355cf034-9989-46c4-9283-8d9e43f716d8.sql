-- Remove 120-minute cap and add 6-hour minimum threshold for gap-filling
-- This allows longer stages to fill large gaps while preventing trivial moves
...
BEGIN
  ...
  -- PHASE 2: GAP-FILLING (NO CAP, 6-hour minimum threshold)
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling (no cap, 6h min threshold)';
  ...
  FOR gap_candidate IN
    ...
    WHERE jsi.schedule_status = 'scheduled'
      ...
      -- REMOVED: AND jsi.scheduled_minutes <= 120 (120-minute cap)
  ...
  LOOP
    ...
    -- Changed from >= 1.0 days to >= 0.25 days (6 hours minimum)
