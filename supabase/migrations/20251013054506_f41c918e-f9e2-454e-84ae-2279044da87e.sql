-- Roll back get_user_accessible_jobs to stable version (pre-"changes requested" view)
-- Clean slate: drop overloads if present
...
-- Recreate the stable 4-argument function with permissive permissions
-- and without any dependency on new views or department mappings
...
BEGIN
  ...
  SELECT DISTINCT ON (pj.id)
    ...
    pj.customer,
    pj.status,
    (pj.due_date)::timestamptz AS due_date, -- explicit cast to match return type
    pj.reference,
    pj.category_id,
    ...
    -- Permissive defaults to avoid permission view dependencies
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE jsi.job_id = pj.id 
      ...
      -- ROLLBACK: remove 'changes_requested' from working set
