-- Fix concurrent workflow issues - corrected SQL syntax
...
WHERE jsi.job_id = jpg.job_id
  AND jsi.job_table_name = jpg.job_table_name
  AND jsi.production_stage_id = ps.id
  AND ps.name ILIKE '%printing%' 
  AND ps.name NOT ILIKE '%queue%'
  AND ps.name NOT ILIKE '%laminating%';

-- 4. Update the standard advance function to handle concurrent activation
...
-- 5. Activate both printing stages for job D425455
UPDATE public.job_stage_instances
SET 
  status = 'active',
  started_at = now(),
