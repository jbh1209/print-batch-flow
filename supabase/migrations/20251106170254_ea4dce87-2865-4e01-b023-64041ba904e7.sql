-- Complete refresh: Restore Oct 24 scheduler, remove divisions, fix wrapper chain
...
-- Step 2: Restore Oct 24 scheduler_append_jobs (NO divisions)
...
BEGIN
  ...
  RAISE NOTICE '🔀 Phase 2: Gap-Filling (up to 3 iterations)';
  ...
  RAISE NOTICE '✅ Complete: % slots, % stages, % gap-filled', wrote_count, updated_count, gap_filled_count;

  RETURN QUERY SELECT wrote_count, updated_count, validation_results;
END;
$$;

-- Step 3: Rewrite scheduler_resource_fill_optimized to directly call scheduler_append_jobs
...
DECLARE
  ...
  result record;
BEGIN
  -- Get all approved, incomplete jobs
  SELECT array_agg(pj.id ORDER BY pj.proof_approved_at ASC)
  INTO job_ids
  FROM production_jobs pj
  ...
  -- Call Oct 24 scheduler with all approved jobs (reflow mode)
  SELECT * INTO result
  FROM scheduler_append_jobs(job_ids, false);
