-- Drop and recreate the get_user_accessible_jobs function with proper column qualification
...
BEGIN
  ...
  WITH job_current_stages AS (
    -- Get the most relevant current stage for each job
  ...
  total_stats AS (
    SELECT 
      jsi_stats.job_id,
      COUNT(jsi_stats.id) as total_stages,
      COUNT(CASE WHEN jsi_stats.status = 'completed' THEN 1 END) as completed_stages
