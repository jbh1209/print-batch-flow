-- Add final synchronization step to ensure JSI times match actual STS times
-- This fixes the data inconsistency between job_stage_instances and stage_time_slots
...
BEGIN
  ...
  RAISE NOTICE 'ENHANCED Sequential Scheduler: Starting from %', base_time;
  ...
  RAISE NOTICE 'ENHANCED Scheduler: Cleared % slots and % instances', clear_result.cleared_slots, clear_result.cleared_instances;
  ...
  RAISE NOTICE 'ENHANCED Scheduler: Initialized % production stages', (SELECT COUNT(*) FROM _stage_tails);
  ...
  LOOP
    ...
    RAISE NOTICE 'ENHANCED: Starting job % (WO: %) - INITIAL barrier: %', 
    ...
    LOOP
      -- Get current job barrier for this job (guarantee never null)
      ...
      RAISE NOTICE 'ENHANCED: Scheduling %: part=%, order=%, duration=%min, earliest_start=%, job_barrier=%, resource_avail=%',
      ...
      job_barriers := jsonb_set(job_barriers, ARRAY[r_job.job_id::text], to_jsonb(actual_end_time));

      RAISE NOTICE 'ENHANCED: Completed stage % - ACTUAL times: %s to %s - UPDATED job barrier to %s',
        r_stage.stage_name, actual_start_time, actual_end_time, actual_end_time;
    END LOOP;
    
    RAISE NOTICE 'ENHANCED: Completed job % - final barrier: %', 
  ...
  RAISE NOTICE 'ENHANCED Sequential Scheduler COMPLETE: % slots written, % instances updated, % synced, % violations remain', 
