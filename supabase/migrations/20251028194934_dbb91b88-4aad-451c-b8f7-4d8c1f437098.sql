-- Surgical fix: Add past-time protection to working Oct 27 scheduler
-- This migration adds ONLY the minimum changes needed to prevent past scheduling
-- while preserving all working barrier logic and shift-aware placement

-- Fix #1: Update place_duration_sql to never schedule in the past
...
BEGIN
  ...
  -- SURGICAL FIX: Start from earliest working moment, but NEVER in the past
  available_start := GREATEST(
    public.next_working_start(p_earliest_start),
    now()
  );
...
-- Fix #2: Update scheduler_reschedule_all_by_division to enforce no-past placement
...
BEGIN
  ...
  -- SURGICAL FIX: Ensure base_time is never in the past
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- SURGICAL FIX: Enforce no-past placement at the stage level
        stage_earliest_start := GREATEST(stage_earliest_start, resource_available_time, now());
...
COMMENT ON FUNCTION public.place_duration_sql IS 'Surgical fix 2025-10-28: Added now() floor to prevent past scheduling';
COMMENT ON FUNCTION public.scheduler_reschedule_all_by_division IS 'Surgical fix 2025-10-28: Added now() floor at base_time and stage_earliest_start to prevent past scheduling';