-- COMPREHENSIVE SCHEDULER FIX: Properly handle completed work and convergence barriers
-- This version eliminates convergence violations by using actual completion times for job barriers
...
BEGIN
  ...
  RAISE NOTICE 'FIXED Sequential Scheduler V2: Starting from %', base_time;
  ...
  RAISE NOTICE 'FIXED Scheduler V2: Cleared % slots and % instances', clear_result.cleared_slots, clear_result.cleared_instances;
  ...
  RAISE NOTICE 'FIXED Scheduler V2: Initialized % production stages', (SELECT COUNT(*) FROM _stage_tails);
  ...
  LOOP
    ...
    RAISE NOTICE 'FIXED V2: Job % (WO: %) - barriers from ACTUAL work: job=%, cover=%, text=%, both=%', 
    ...
    LOOP
      ...
      IF r_stage.part_assignment = 'both' THEN
        ...
        BEGIN
          -- CRITICAL FIX: Get the ACTUAL part completion times for this job from job_part_barriers
          job_part_data := job_part_barriers -> r_job.job_id::text;
          
          -- CRITICAL FIX: Use the ACTUAL part barriers (from completed or scheduled work)
          ...
          -- Use the LATER of the two completion times, and NEVER earlier than job_current_barrier
          convergence_barrier := GREATEST(cover_end_time, text_end_time, job_current_barrier);
          
          RAISE NOTICE 'CONVERGENCE BARRIER: Stage % waits for cover=%s, text=%s, job_barrier=%s -> USING convergence_barrier=%s',
            r_stage.stage_name, cover_end_time, text_end_time, job_current_barrier, convergence_barrier;
          
          -- OVERRIDE job_current_barrier for this 'both' stage
      ...
      RAISE NOTICE 'FIXED V2: Scheduling %: part=%, order=%, duration=%min, earliest_start=%, job_barrier=%, resource_avail=%',
      ...
      RAISE NOTICE 'FIXED V2: Completed stage % - ACTUAL times: %s to %s - ADDED to layer %s',
  ...
  RAISE NOTICE 'FIXED Sequential Scheduler V2 COMPLETE: % slots written, % instances updated, % synced, % violations remain', 
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'FIXED Sequential Scheduler V2 failed: %', SQLERRM;
END;
$function$;