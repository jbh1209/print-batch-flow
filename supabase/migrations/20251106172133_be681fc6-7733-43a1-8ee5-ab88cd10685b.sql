-- Emergency fix: Replace broken scheduler_append_jobs with actual working Oct 14 version
-- The previous migration fabricated a non-existent version. This restores the real working code.

DROP FUNCTION IF EXISTS public.scheduler_append_jobs(uuid[], boolean);

-- COMPLETE WORKING VERSION from 20251014123106_bc972dd9-96c2-447d-bfa3-926c8c128e81.sql
...
-- Fix scheduler_resource_fill_optimized to call the working function correctly
...
AS $function$
DECLARE
  all_incomplete_job_ids uuid[];
BEGIN
  -- Get all approved, incomplete job IDs
  SELECT ARRAY_AGG(DISTINCT pj.id)
  INTO all_incomplete_job_ids
  FROM production_jobs pj
  WHERE pj.proof_approved_at IS NOT NULL
    AND pj.status NOT IN ('completed', 'cancelled', 'invoiced', 'on_hold')
    AND EXISTS (
  ...
  RAISE NOTICE '🔄 Full reschedule: calling scheduler_append_jobs for % jobs', array_length(all_incomplete_job_ids, 1);

  -- Call the working scheduler_append_jobs
  RETURN QUERY
  SELECT * FROM public.scheduler_append_jobs(all_incomplete_job_ids, p_only_if_unset);
END;
$function$;