-- Drop and recreate scheduler_reschedule_all_parallel_aware with predecessor constraint
...
BEGIN
  ...
  -- Validate no sequence violations exist after Phase 1
  DECLARE
  ...
  BEGIN
    ...
    FROM job_stage_instances jsi
    LEFT JOIN LATERAL (
      SELECT MAX(jsi2.scheduled_end_at) as max_end
      FROM job_stage_instances jsi2
    ...
    WHERE jsi.scheduled_start_at IS NOT NULL
      AND prev_stages.max_end IS NOT NULL
      AND jsi.scheduled_start_at < prev_stages.max_end;
    
    IF violation_count > 0 THEN
      RAISE WARNING '⚠️ Phase 1 completed with % sequence violations!', violation_count;
    ELSE
      RAISE NOTICE '✅ Phase 1: 0 sequence violations detected';
    END IF;
  END;
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL AND best_gap.days_earlier >= 0.25 THEN
      ...
      RAISE NOTICE '🔀 Gap-fill candidate: Job % (WO: %), Stage %, Original: %, Gap: %, Days saved: %',
        gap_candidate.job_id, gap_candidate.wo_no, gap_candidate.stage_name, 
        original_start, best_gap.gap_start, days_saved;
      
      -- Use place_duration_sql to handle multi-day scheduling properly
      SELECT * INTO placement_result
      ...
      IF placement_result.placement_success AND placement_result.slots_created IS NOT NULL THEN
        ...
        -- Update stage instance with new times
        UPDATE job_stage_instances
        SET 
        ...
        RAISE NOTICE '✅ Gap-filled: Job % (WO: %), Stage %, %.2f days saved',
          gap_candidate.job_id, gap_candidate.wo_no, gap_candidate.stage_name, days_saved;
      END IF;
    END IF;
  END LOOP;
  ...
  validation_results := (
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'scheduled_start', v.scheduled_start_at,
        'expected_after', v.max_predecessor_end,
        'violation_type', 'predecessor_constraint'
      )
    ), '[]'::jsonb)
    FROM (
      SELECT 
        ...
        ps.name as stage_name,
        jsi.scheduled_start_at,
        prev.max_predecessor_end
      FROM job_stage_instances jsi
      JOIN production_jobs pj ON pj.id = jsi.job_id
      JOIN production_stages ps ON ps.id = jsi.production_stage_id
      LEFT JOIN LATERAL (
        SELECT MAX(jsi2.scheduled_end_at) as max_predecessor_end
        FROM job_stage_instances jsi2
        WHERE jsi2.job_id = jsi.job_id
          AND jsi2.stage_order < jsi.stage_order
          AND jsi2.scheduled_end_at IS NOT NULL
      ...
      WHERE jsi.scheduled_start_at IS NOT NULL
        AND prev.max_predecessor_end IS NOT NULL
        AND jsi.scheduled_start_at < prev.max_predecessor_end
    ) v
  );
