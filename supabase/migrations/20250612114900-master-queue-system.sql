...
-- Update the get_user_accessible_jobs function to handle master queue consolidation
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = p_user_id AND role = 'admin') THEN
    ...
    SELECT 
      ...
      COALESCE(ps.master_queue_id, ps.id)::uuid as master_queue_id,
      CASE 
        WHEN ps.master_queue_id IS NOT NULL THEN 
          (SELECT mq.name FROM public.production_stages mq WHERE mq.id = ps.master_queue_id)
        ELSE ps.name 
    ...
    WHERE 
      ...
      AND (p_stage_filter IS NULL OR COALESCE(ps.master_queue_id, ps.id) = p_stage_filter)
    ORDER BY pj.wo_no;
  ELSE
    -- Non-admin users - use existing permission logic with master queue support
    RETURN QUERY
    WITH user_groups AS (
      SELECT ugm.group_id
