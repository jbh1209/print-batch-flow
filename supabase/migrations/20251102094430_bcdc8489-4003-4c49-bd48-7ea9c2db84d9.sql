-- Bulk complete all DTP stages and advance to next stage
-- This handles both active DTP stages and jobs where DTP is the next pending stage
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE '🚀 STARTING BULK DTP COMPLETION PROCESS';
  RAISE NOTICE '========================================';
  ...
  LOOP
    BEGIN
      RAISE NOTICE '  ⚙️  Processing active DTP job: % (stage_instance: %)', v_rec.wo_no, v_rec.stage_instance_id;
      
      -- Call the advance function with parallel support
      SELECT public.advance_job_stage_with_parallel_support(
        v_rec.job_id,
        v_rec.job_table_name,
        v_rec.stage_instance_id,
  ...
  -- STEP 2: Process jobs where DTP is the NEXT PENDING stage (activate then advance)
  RAISE NOTICE '';
  ...
  LOOP
    BEGIN
      RAISE NOTICE '  ⚙️  Processing pending DTP job: % (stage_instance: %)', v_rec.wo_no, v_rec.stage_instance_id;
...
-- Verification query: Check remaining DTP stages after bulk completion
SELECT 
  ps.name as stage_name,
  jsi.status,
  COUNT(*) as count
