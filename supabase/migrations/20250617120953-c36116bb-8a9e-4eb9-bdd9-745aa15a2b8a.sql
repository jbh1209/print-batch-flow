...
-- Fix the database function to only show jobs that are CURRENTLY ACTIVE in the user's accessible stages
-- Not jobs that are assigned to those stages but still in earlier workflow steps
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = p_user_id AND role = 'admin') THEN
    ...
    ORDER BY pj.wo_no;
  ELSE
    -- CRITICAL FIX: Only show jobs where the user's accessible stage is the CURRENT ACTIVE stage
    RETURN QUERY
    WITH user_groups AS (
      SELECT ugm.group_id
    ...
    -- CRITICAL FIX: Only get jobs where the current active stage matches user permissions
    -- This ensures we only show jobs ready to work on, not future workload
    current_active_jobs AS (
      ...
      WHERE jsi.job_table_name = 'production_jobs'
        AND jsi.status = 'active'  -- ONLY ACTIVE stages (not pending future stages)
      ORDER BY jsi.job_id, jsi.stage_order ASC
    ),
    job_stage_counts AS (
      SELECT 
      ...
      WHERE aj_jsi.job_table_name = 'production_jobs'
        AND aj_jsi.job_id IN (SELECT caj.job_id FROM current_active_jobs caj)
    ...
    FROM public.production_jobs pj
    INNER JOIN current_active_jobs caj ON pj.id = caj.job_id
    LEFT JOIN job_stage_counts jsc ON pj.id = jsc.job_id
