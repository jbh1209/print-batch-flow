-- TIGHT PACKING FIX: Update find_available_gaps to support alignment within gaps
...
BEGIN
  ...
  WHILE current_day < p_fifo_start_time::date LOOP
    ...
    LOOP
      -- NEW: Apply alignment if requested
...
-- TIGHT PACKING FIX: Update Phase 2 gap-filling to use aligned placement
...
BEGIN
  ...
  -- ========== PHASE 2: TIGHT PACKING GAP-FILLING ==========
  RAISE NOTICE '🔀 Starting Phase 2: Tight Packing Gap-Filling (6-hour threshold)';
  
  -- Allow near-term improvements
  ...
  LOOP
    ...
    -- TIGHT PACKING: Find best aligned gap that respects earliest_possible_start
    SELECT g.*
    INTO best_gap
    FROM find_available_gaps(
      ...
      original_start,
      v_lookback_days,
      earliest_possible_start  -- NEW: Pass alignment point
    ) g
    ORDER BY g.gap_start ASC
    LIMIT 1;
    
    -- TIGHT PACKING: Use aligned placement directly
    ...
    IF best_gap IS NOT NULL 
      ...
      RAISE NOTICE '🔀 GAP-FILLING (TIGHT): Moving stage % from % to % (saves %.2f days / %.1f hours; earliest_possible=%s; gap=[% → %]; finishing=%, cap=%d, lookback=%d)',
        gap_candidate.stage_name,
        original_start,
        new_start,
        days_saved,
      ...
      -- Create new gap-filled slot using aligned placement
      INSERT INTO stage_time_slots(
        production_stage_id,
        date,
        slot_start_time,
      ...
      -- Update job_stage_instances using aligned placement
      UPDATE job_stage_instances
      SET 
  ...
  RAISE NOTICE '✅ Tight packing gap-filling complete: % stages moved (6-hour threshold)',
    gap_filled_count;
