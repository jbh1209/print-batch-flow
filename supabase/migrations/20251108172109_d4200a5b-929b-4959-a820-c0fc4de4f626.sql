-- ============================================================
-- MIGRATION: Rename return key to 'updated_jsi' for scheduler function
-- Context: simple_scheduler_wrapper expects 'updated_jsi'
-- This updates the JSON response from scheduler_reschedule_all_parallel_aware
-- by replacing 'scheduled_count' key with 'updated_jsi'
-- ============================================================
...
-- Recreate the function with the same logic, but updated return key
...
BEGIN
  ...
  -- Pre-aggregate completed barriers
  barrier_agg AS (
    SELECT 
      ss.id,
      jsonb_object_agg(
    ...
    LEFT JOIN LATERAL (
      ...
      FROM public.job_stage_instances pred
      JOIN public.production_stages ps ON ps.id = pred.production_stage_id
  ...
  SELECT 
    ...
    COALESCE(
      ...
      ) FILTER (WHERE su.days_saved >= 0.25),
      '[]'::jsonb
    )
