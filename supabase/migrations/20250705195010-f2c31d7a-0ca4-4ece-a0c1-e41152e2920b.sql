-- Fix master job workflow initialization to include ALL category stages
...
BEGIN
  ...
  -- Initialize master job with COMPLETE category workflow
  ...
  LOOP
    INSERT INTO public.job_stage_instances (
      ...
      CASE 
        WHEN stage_record.production_stage_id = printing_stage_id THEN 'active' -- Start at printing
        ELSE 'pending' 
      END,
      CASE 
        WHEN stage_record.production_stage_id = printing_stage_id THEN now()
        ELSE NULL 
      END,
      CASE 
        WHEN stage_record.production_stage_id = printing_stage_id THEN p_created_by
        ELSE NULL 
      END
    );
  END LOOP;
