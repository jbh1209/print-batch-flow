...
-- Fix the get_user_accessible_jobs function to properly filter completed jobs by default
-- This ensures the single source of truth for job completion is respected
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = p_user_id AND role = 'admin') THEN
    ...
    WHERE 
      -- CRITICAL FIX: Default behavior excludes completed jobs unless specifically requested
    ...
    ORDER BY pj.wo_no;
  ELSE
    -- Non-admin users - use existing permission logic with completion filter
    RETURN QUERY
    WITH user_groups AS (
      SELECT ugm.group_id
