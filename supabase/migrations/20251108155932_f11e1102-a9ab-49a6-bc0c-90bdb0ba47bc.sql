-- Fix incorrect parameter order when calling find_available_gaps in Phase 2
-- of scheduler_reschedule_all_parallel_aware. Swaps the 4th and 5th args to
-- match the function signature: (uuid, integer, timestamptz, integer, timestamptz)

CREATE OR REPLACE FUNCTION public.scheduler_reschedule_all_parallel_aware(p_start_from timestamp with time zone DEFAULT now())
RETURNS TABLE(wrote_slots integer, updated_jsi integer, violations jsonb, gap_filled_count integer)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR pass_iteration IN 1..3 LOOP
    ...
    LOOP
      ...
      -- Find best available gap (fixed arg order)
