-- CRITICAL FIX: Phase 2 Gap-Filling with Precedence Alignment
-- This migration surgically replaces ONLY the Phase 2 gap-filling loop in scheduler_reschedule_all_parallel_aware
-- DO NOT modify Phase 1 FIFO scheduling logic
-- DO NOT modify helper functions
-- DO NOT modify any other database objects
...
BEGIN
  ...
  -- PHASE 2: GAP-FILLING WITH PRECEDENCE ALIGNMENT (FIXED)
  -- INSTRUCTION: This phase MUST use the 5-argument find_available_gaps call with p_align_at parameter
  -- INSTRUCTION: Acceptance condition MUST be: gap_start >= earliest_possible_start AND gap_start < original_start
  RAISE NOTICE '🔀 Phase 2: Gap-Filling (searching forward from predecessor completion with alignment)';
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL 
      ...
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % (WO: %, order %) from % to % (saves %.2f days, earliest_possible: %)',
