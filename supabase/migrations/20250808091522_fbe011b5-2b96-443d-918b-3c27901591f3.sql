-- Fix variable name ambiguity in advance_parallel_job_stage function
CREATE OR REPLACE FUNCTION public.advance_parallel_job_stage(p_job_id uuid, p_job_table_name text, p_current_stage_id uuid, p_completed_by uuid DEFAULT auth.uid(), p_notes text DEFAULT NULL::text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  ...
  v_actual_duration_minutes INTEGER; -- Renamed to avoid ambiguity with table column
...
BEGIN
  ...
  SET 
    ...
    actual_duration_minutes = v_actual_duration_minutes, -- Use renamed variable
  ...
  -- Find and activate the next sequential stage(s)
