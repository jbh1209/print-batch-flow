...
DECLARE
  ...
  new_order INTEGER;
BEGIN
  -- Loop through each category that has issues
  FOR cat_record IN 
  ...
  -- Also fix any categories with gaps in ordering
  FOR cat_record IN 
    SELECT DISTINCT category_id 
    FROM public.category_production_stages
  LOOP
    ...
    -- Re-sequence all stages for this category to ensure no gaps
    FOR stage_record IN 
      SELECT id 
      FROM public.category_production_stages 
      WHERE category_id = cat_record.category_id 
    ...
    LOOP
      UPDATE public.category_production_stages 
      SET stage_order = new_order, updated_at = now() 
      WHERE id = stage_record.id AND stage_order != new_order;
