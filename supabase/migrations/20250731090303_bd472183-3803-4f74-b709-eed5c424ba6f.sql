-- Enhanced function for advancing parallel workflow stages
...
BEGIN
  ...
  SELECT jsi.*, ps.name as stage_name, jsi.dependency_group
  ...
  IF dependency_group_id IS NOT NULL THEN
    ...
    -- Only proceed to next stages if ALL stages in the dependency group are completed
