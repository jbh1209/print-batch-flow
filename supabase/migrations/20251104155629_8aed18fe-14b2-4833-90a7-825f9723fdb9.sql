-- Update scheduler_reschedule_all_parallel_aware to prevent slot collisions and improve observability
...
BEGIN
  ...
  -- ========== STEP 1: Capture completed barriers BEFORE wiping ==========
  completed_barriers := (
    SELECT jsonb_object_agg(
      agg.job_id::text,
      jsonb_build_object(
        'main',  COALESCE(agg.main_end,  base_time),
        'cover', COALESCE(agg.cover_end, base_time),
        'text',  COALESCE(agg.text_end,  base_time),
        'both',  COALESCE(agg.both_end,  base_time)
      )
    )
    FROM (
      SELECT
        jsi.job_id,
        MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment IN ('both'))          AS both_end,
        ...
        MAX(sts.slot_end_time) FILTER (WHERE jsi.part_assignment IN ('both'))          AS main_end
  ...
  -- ========== STEP 2: Nuclear wipe - totally clean slate ==========
  -- Previously: DELETE FROM stage_time_slots WHERE slot_start_time >= base_time;
  DELETE FROM stage_time_slots;
  -- ========== END STEP 2 ==========

  -- Expire old scheduled/auto_held stages older than 7 days
  ...
  -- Reset status for pending/on_hold (except expired)
  ...
  -- Build availability tracker and tails from completed history
  ...
  -- Ensure all stages have a tail record at least at base_time
  INSERT INTO _stage_tails(stage_id, next_available_time)
  SELECT DISTINCT jsi.production_stage_id, base_time
  FROM job_stage_instances jsi
  ON CONFLICT (stage_id) DO NOTHING;

  -- Phase 1: Place all stages in FIFO order (proof_approved_at), respecting resource tails
  FOR r_job IN
    SELECT 
      pj.id as job_id,
      pj.wo_no,
  ...
  LOOP
    -- Initialize per-job barriers from completed history or proof approval
    ...
    LOOP
      ...
      LOOP
        ...
        -- Determine earliest start by job barriers
        ...
        -- Prevent collisions while keeping FIFO via job barriers
        stage_earliest_start := GREATEST(stage_earliest_start, resource_available_time);

        -- Instrumentation: log intended placement window
        RAISE NOTICE '📅 Plan job %, wo %, stage_inst %, prod_stage %, earliest %, tail %',
          r_job.job_id, r_job.wo_no, r_stage.stage_instance_id, r_stage.production_stage_id,
          stage_earliest_start, resource_available_time;

        -- Place duration into working windows
        ...
        -- Instrumentation: log actual placement start and end
        SELECT MIN((time_slot ->> 'start_time')::timestamptz), MAX((time_slot ->> 'end_time')::timestamptz)
        INTO original_start, stage_end_time
        FROM jsonb_array_elements(placement_result.slots_created) time_slot;

        RAISE NOTICE '🧩 Placed job %, wo %, stage_inst %, prod_stage %, start %, end %, mins %',
          r_job.job_id, r_job.wo_no, r_stage.stage_instance_id, r_stage.production_stage_id,
          original_start, stage_end_time, r_stage.duration_minutes;
      ...
      -- Consolidate main barrier to the max of all parts to retain FIFO within a job
      SELECT GREATEST(
