-- Migration: Remove 120-minute duration cap from gap-filling logic
-- This allows long-duration finishing stages (Perfect Binding, Cutting, etc.) to be gap-filled
...
-- Recreate scheduler_append_jobs WITHOUT 120-minute cap
...
BEGIN
  ...
  -- ========== PHASE 2: GAP-FILLING (NO DURATION CAP) ==========
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling Optimization Pass for appended jobs';
  
  -- REMOVED: jsi.scheduled_minutes <= 120 constraint
...
-- Recreate scheduler_reschedule_all_parallel_aware WITHOUT 120-minute cap
