-- Create a record type for scheduler results
CREATE TYPE scheduler_result_type AS (
  wrote_slots integer,
  updated_jsi integer,
  violations jsonb
);
...
-- Recreate the function to return a single record instead of TABLE
...
BEGIN
  ...
  -- This ensures maximum shift utilization
  FOR r_stage IN
    WITH stage_queue AS (
      SELECT 
