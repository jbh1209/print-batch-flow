-- =====================================================================
-- COMPLETE RESTORATION OF OCTOBER 12TH WORKING SCHEDULER
-- =====================================================================
-- This migration explicitly DROPS all stub versions and restores the 
-- proven working scheduler from October 12, 2024.
-- DO NOT MODIFY THIS SCHEDULER - it is the baseline that works correctly.
-- Division awareness will be added AFTER verification.
-- =====================================================================

-- 1. Drop any existing stub versions of the scheduler
DROP FUNCTION IF EXISTS public.scheduler_reschedule_all_parallel_aware(TIMESTAMPTZ, TEXT, BOOLEAN) CASCADE;
...
-- 2. Restore the COMPLETE October 12th scheduler (EXACT COPY from migration 20251012085539)
...
BEGIN
    ...
    LOOP
      ...
      LOOP
        ...
        IF predecessor_end IS NOT NULL AND predecessor_end > stage_earliest_start THEN
          RAISE NOTICE '🔒 Job % (WO: %) stage % (order %): waiting for predecessor to finish. Barrier time: %, Predecessor end: %, New earliest: %',
        ...
        -- PHASE 1 CONFLICT-SAFE INSERTION
  ...
  -- Multi-pass loop: allow improvements to propagate through dependent stages
  FOR pass_iteration IN 1..3 LOOP
    ...
    FOR gap_candidate IN
      SELECT 
        ...
        pj.wo_no,
        -- CRITICAL: Calculate earliest_possible_start from predecessor completion
        COALESCE(
      ...
      -- INSTRUCTION: Process predecessors first, then by job, then by time
      ORDER BY 
        jsi.job_id,
        jsi.stage_order ASC,
    ...
    LOOP
      ...
      -- INSTRUCTION: Call find_available_gaps with ALL 5 arguments including p_align_at
      ...
      FROM find_available_gaps(
        ...
        original_start,
        v_lookback_days,
        earliest_possible_start  -- p_align_at: ensures gaps respect precedence
      )
      WHERE gap_start >= earliest_possible_start
      ORDER BY gap_start ASC
      LIMIT 1;
      
      -- INSTRUCTION: Accept gap if it exists AND is between predecessor end and FIFO scheduled time
      IF best_gap IS NOT NULL 
         AND best_gap.gap_start >= earliest_possible_start 
         AND best_gap.gap_start < original_start THEN
        
        -- Calculate days saved explicitly
        days_saved := EXTRACT(EPOCH FROM (original_start - best_gap.gap_start)) / 86400.0;
        
        RAISE NOTICE '🔀 GAP-FILLING Pass %: Moving stage % (WO: %, order %) from % to % (saves %.2f days, earliest_possible: %)',
        ...
        IF placement_result.placement_success THEN
          -- PHASE 2 CONFLICT-SAFE INSERTION
          FOR slot_record IN SELECT * FROM jsonb_array_elements(placement_result.slots_created)
          LOOP
            INSERT INTO stage_time_slots(
              production_stage_id, date, slot_start_time, slot_end_time,
          ...
          -- Derive actual end time from all slots
          SELECT MIN(slot_start_time), MAX(slot_end_time)
          ...
          END IF;
        ELSE
          RAISE WARNING '⚠️ Failed to place stage % into gap starting at %', gap_candidate.stage_name, best_gap.gap_start;
        END IF;
      END IF;
    END LOOP;
    
    RAISE NOTICE '✅ Phase 2 Pass %/3 complete: % stages moved this pass', pass_iteration, moved_count;
    
    -- Early exit if no movements occurred
    IF moved_count = 0 THEN
      RAISE NOTICE '⏹️ Convergence reached early: no movements in pass %', pass_iteration;
      EXIT;
    END IF;
  END LOOP;
  ...
  -- PHASE 3: VALIDATION
  RAISE NOTICE '🔍 Phase 3: Validation starting...';
  ...
  RAISE NOTICE '✅ Scheduler complete. Wrote % slots, updated % stages, logged % gap fills, found % validation notes',
    wrote_count, updated_count, gap_filled_count, jsonb_array_length(validation_results);

  RETURN QUERY SELECT wrote_count, updated_count, validation_results;
END;
$function$;

-- 3. Update the wrapper to call the restored October 12th scheduler
...
BEGIN
  -- For now, ignore p_division and just call the proven working October 12th scheduler
  -- Division awareness will be added AFTER we verify this works correctly
  SELECT r.wrote_slots, r.updated_jsi, COALESCE(r.violations, '[]'::jsonb)
    INTO core_wrote, core_updated, core_violations
  FROM public.scheduler_reschedule_all_parallel_aware(p_start_from) AS r;
  
  -- Return multiple result formats for compatibility with different callers
...
-- 5. Add protective comments
COMMENT ON FUNCTION public.scheduler_reschedule_all_parallel_aware IS 'PROVEN WORKING SCHEDULER FROM OCT 12, 2024 - Features: (1) Schedules from TOMORROW not today, (2) Enforces sequential predecessor dependencies, (3) Uses place_duration_sql for shift-aware placement. DO NOT MODIFY WITHOUT TESTING.';
COMMENT ON FUNCTION public.simple_scheduler_wrapper IS 'Wrapper for scheduler - currently ignoring division parameter until October 12th baseline is verified working. Then division awareness will be surgically added.';