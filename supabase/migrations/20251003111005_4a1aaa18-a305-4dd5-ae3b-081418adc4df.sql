-- PRECISE FIX: Reduce gap-fill savings threshold from 1 day to 6 hours (0.25 days)
-- This allows same-day improvements while preventing trivial micro-optimizations
...
DECLARE
  ...
  -- ENHANCED: More aggressive gap-filling constraints
...
BEGIN
  ...
  -- ========== PHASE 2: ENHANCED GAP-FILLING WITH AGGRESSIVE LOOKBACK ==========
  RAISE NOTICE '🔀 Starting Phase 2: Enhanced Gap-Filling (100%% finishing, 70%% others, 6-hour threshold)';
  
  -- ENHANCED: Reduce minimum future threshold from 2 days to 1 day
  v_min_future_threshold := now() + interval '1 day';
  
  -- UPDATED: Process stages by stage_order ASCENDING first, then by scheduled_start_at DESC
  ...
  LOOP
    ...
    -- ENHANCED: Identify if this is a finishing-type stage
    ...
    -- UPDATED: Movement cap based on stage type
    ...
    -- ENHANCED: More aggressive lookback for finishing stages (100% up to 30 days)
    -- and 70% for other stages (up to 30 days)
    v_days_until_original := EXTRACT(epoch FROM (original_start - now())) / 86400.0;
    
    IF v_is_finishing_stage THEN
      -- Finishing stages: 100% of scheduling distance (up to 30 days)
      v_lookback_days := LEAST(30, GREATEST(7, FLOOR(v_days_until_original)::integer));
    ELSE
      -- Other stages: 70% of scheduling distance (up to 30 days)
    ...
    -- Find best gap with enhanced lookback
    ...
    -- CRITICAL FIX: Changed threshold from 1.0 days to 0.25 days (6 hours minimum)
    -- This allows same-day improvements while preventing trivial micro-optimizations
    IF best_gap IS NOT NULL 
       AND best_gap.days_earlier >= 0.25 
       AND best_gap.days_earlier <= v_stage_move_cap THEN
      
      days_saved := best_gap.days_earlier;
      hours_saved := days_saved * 24;
      
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % from % to % (saves %.2f days / %.1f hours, finishing=%, cap=%d, lookback=%d)',
        gap_candidate.stage_name,
        original_start,
        best_gap.gap_start,
        days_saved,
    ...
    ELSIF best_gap IS NOT NULL AND best_gap.days_earlier > v_stage_move_cap THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: would move %.1f days (exceeds cap %d)',
        gap_candidate.stage_name, best_gap.days_earlier, v_stage_move_cap;
    ELSIF best_gap IS NOT NULL AND best_gap.days_earlier < 0.25 THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: would only save %.2f days / %.1f hours (below 6-hour threshold)',
        gap_candidate.stage_name, best_gap.days_earlier, best_gap.days_earlier * 24;
  ...
  RAISE NOTICE '✅ Enhanced gap-filling complete: % stages moved (finishing: 100%%, others: 70%%, threshold: 6 hours)',
    gap_filled_count;
