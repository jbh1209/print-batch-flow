-- Fix variable reuse bug in scheduler_append_jobs
-- Bug: slot_record (jsonb) was incorrectly reused to store UUID from RETURNING id
-- This caused "invalid input syntax for type json" error when trying to assign UUID to JSONB variable
...
DECLARE
  ...
  slot_record jsonb;
  inserted_slot_id uuid;  -- ✅ NEW: Separate variable for RETURNING id
...
BEGIN
  ...
  -- Initialize resource availability from ALL future slots (completed + pending)
    ...
    LOOP
      ...
      LOOP
        ...
        LOOP
          ...
          RETURNING id INTO inserted_slot_id;  -- ✅ FIXED: Use UUID variable instead of JSONB
          
          IF inserted_slot_id IS NOT NULL THEN  -- ✅ FIXED: Check UUID variable
            job_inserted_slot_ids := array_append(job_inserted_slot_ids, inserted_slot_id);  -- ✅ FIXED: Use UUID directly
        ...
        -- Update resource availability with 1-minute safety buffer
        UPDATE _stage_tails 
