...
-- Create the definitive version with 4 parameters as expected by frontend
...
RETURNS TABLE(
  job_id uuid, id uuid, wo_no text, customer text, status text, due_date text, reference text, 
  category_id uuid, category_name text, category_color text, current_stage_id uuid, 
  ...
  workflow_progress numeric, total_stages integer, completed_stages integer, display_stage_name text, 
  qty integer, has_custom_workflow boolean, manual_due_date text, batch_category text, 
  is_in_batch_processing boolean, started_by uuid, started_by_name text, proof_emailed_at timestamp with time zone, 
  is_batch_master boolean, batch_name text, constituent_job_count integer, current_stage_order integer, 
  is_virtual_stage_entry boolean, stage_instance_id uuid, parent_job_id uuid, part_assignment text
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
BEGIN
  ...
  SELECT 
    ...
    COALESCE(MAX(ugsp.can_view), false) as user_can_view,
    COALESCE(MAX(ugsp.can_edit), false) as user_can_edit,
    COALESCE(MAX(ugsp.can_work), false) as user_can_work,
    COALESCE(MAX(ugsp.can_manage), false) as user_can_manage,
    COALESCE(
      ROUND(
