-- Complete system restoration - remove all multi-part/concurrent functionality
...
-- Step 7: Ensure proper stage ordering in categories
-- Update stage orders to ensure Batch Allocation appears in correct position
UPDATE public.category_production_stages cps
SET stage_order = ps.order_index
FROM public.production_stages ps
WHERE cps.production_stage_id = ps.id
AND ps.order_index IS NOT NULL;