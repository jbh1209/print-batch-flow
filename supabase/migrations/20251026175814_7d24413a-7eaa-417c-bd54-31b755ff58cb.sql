-- Add division awareness to scheduler_append_jobs and update all triggers to pass division

-- Step 1: Update scheduler_append_jobs to accept p_division parameter
...
BEGIN
  ...
  RAISE NOTICE '🔄 Appending jobs starting from TOMORROW: % (division filter: %)', base_time, COALESCE(p_division, 'ALL');
  ...
  RAISE NOTICE '📋 Phase 1: FIFO Scheduling for % jobs (division: %)...', array_length(p_job_ids, 1), COALESCE(p_division, 'ALL');
...
-- Step 2: Update stage-level proof approval trigger to pass division
...
BEGIN
  IF NEW.proof_approved_manually_at IS NOT NULL AND (OLD.proof_approved_manually_at IS NULL OR OLD.proof_approved_manually_at IS DISTINCT FROM NEW.proof_approved_manually_at) THEN
    BEGIN
      ...
      SELECT pj.division INTO v_division 
      FROM production_jobs pj 
      ...
      RAISE NOTICE '✅ [STAGE] Scheduler completed: % slots written, % stages updated (division: %)', 
        v_result.wrote_slots, v_result.updated_jsi, COALESCE(v_division, 'NULL');
      ...
      SELECT 
        ...
        format('Division-aware scheduler_append_jobs: wrote %s slots, updated %s stages, division: %s', 
          v_result.wrote_slots, v_result.updated_jsi, COALESCE(v_division, 'NULL'))
...
-- Step 3: Update job-level proof approval trigger to pass division
...
BEGIN
  IF NEW.proof_approved_at IS NOT NULL AND (OLD.proof_approved_at IS NULL OR OLD.proof_approved_at IS DISTINCT FROM NEW.proof_approved_at) THEN
    BEGIN
      ...
      RAISE NOTICE '🚀 [JOB] Proof approval trigger for job % (WO: %) - %s stages (division: %)', 
        NEW.id, NEW.wo_no, v_stage_count, COALESCE(NEW.division, 'NULL');
      ...
      RAISE NOTICE '✅ [JOB] Scheduler completed: % slots written, % stages updated (division: %)', 
        v_result.wrote_slots, v_result.updated_jsi, COALESCE(NEW.division, 'NULL');
      ...
      VALUES (
        ...
        format('Division-aware scheduler_append_jobs: wrote %s slots, updated %s stages. Division: %s, Stages count: %s', 
          v_result.wrote_slots, v_result.updated_jsi, COALESCE(NEW.division, 'NULL'), v_stage_count)
      );
