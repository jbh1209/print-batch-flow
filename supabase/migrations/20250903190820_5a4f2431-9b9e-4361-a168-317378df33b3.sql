-- Fix barrier initialization for completed stages in scheduler_reschedule_all_parallel_aware
-- This ensures completed stages with NULL scheduled_end_at but valid completed_at times
-- are properly used for dependency calculations
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- CRITICAL FIX: Ensure barrier exists and use it
        ...
        RAISE NOTICE 'Scheduling stage % (%): % mins from % (barrier: % = %)',
          ...
          stage_earliest_start, barrier_key, (job_stage_barriers ->> barrier_key);
