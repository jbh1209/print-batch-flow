-- Fix gap-filling to place stages at max(gap_start, earliest_possible_start) for tight packing
-- This allows stages to fit precisely within existing gaps when predecessors finish mid-gap
...
DECLARE
  ...
  -- Variables for proper combined stage handling
  ...
  -- ENHANCED: Capacity-aware gap placement
  earliest_possible_start timestamptz;
  new_start timestamptz;
  new_end timestamptz;
  v_min_future_threshold timestamptz;
  v_stage_move_cap integer;
  v_lookback_days integer;
  v_days_until_original numeric;
  v_is_finishing_stage boolean;
BEGIN
  ...
  -- Only clear scheduling data for non-completed stages
  ...
  LOOP
    -- Initialize barriers from already-completed stages in this job
    ...
    LOOP
      ...
      FOR r_stage IN
        ...
        ORDER BY jsi.id
      LOOP
        -- Proper barrier calculation for combined stages
        ...
        -- Update barriers correctly based on part assignment
        IF r_stage.part_assignment = 'both' THEN
          job_stage_barriers := job_stage_barriers 
  ...
  -- ========== PHASE 2: CAPACITY-AWARE GAP-FILLING WITH TIGHT PACKING ==========
  RAISE NOTICE '🔀 Starting Phase 2: Capacity-Aware Gap-Filling (tight packing enabled, 6-hour threshold)';
  
  -- CHANGE 1: Allow near-term improvements (changed from now() + 1 day to now())
  v_min_future_threshold := now();
  
  -- Process stages by stage_order ASCENDING first, then by scheduled_start_at DESC
  ...
  LOOP
    ...
    -- Get earliest possible start time based on predecessor stages in the same job
    ...
    -- Identify if this is a finishing-type stage
    v_is_finishing_stage := gap_candidate.stage_name ILIKE ANY(ARRAY[
      '%trimming%', '%packaging%', '%boxing%', '%dispatch%', '%shipping%',
      '%gathering%', '%binding%', '%cutting%', '%finishing%'
    ]);
    
    -- Movement cap based on stage type
    IF v_is_finishing_stage THEN
      v_stage_move_cap := 30;
    ELSE
      v_stage_move_cap := 21;
    END IF;
    ...
    v_days_until_original := EXTRACT(epoch FROM (original_start - now())) / 86400.0;
    
    IF v_is_finishing_stage THEN
      v_lookback_days := LEAST(30, GREATEST(7, FLOOR(v_days_until_original)::integer));
    ELSE
      v_lookback_days := LEAST(30, GREATEST(7, FLOOR(v_days_until_original * 0.7)::integer));
    END IF;
    
    RAISE NOTICE '🔀 Gap-fill candidate: stage % (order %, finishing=%): original=%, earliest_possible=%, move_cap=%d days, lookback=%d days',
      gap_candidate.stage_name,
      gap_candidate.stage_order,
      v_is_finishing_stage,
      original_start,
      earliest_possible_start,
      v_stage_move_cap,
      v_lookback_days;
    
    -- CHANGE 2: Capacity-aware gap selection that orders by computed candidate start
    SELECT g.*
    INTO best_gap
    ...
    WHERE g.gap_end >= earliest_possible_start + make_interval(mins => gap_candidate.scheduled_minutes)
    ORDER BY GREATEST(g.gap_start, earliest_possible_start) ASC, g.gap_start ASC
    LIMIT 1;
    
    -- CHANGE 3: Align placement to earliest_possible_start within the gap
    IF best_gap IS NOT NULL THEN
      new_start := GREATEST(best_gap.gap_start, earliest_possible_start);
      new_end := new_start + make_interval(mins => gap_candidate.scheduled_minutes);
      
      -- Recompute savings using the true placement
      days_saved := EXTRACT(epoch FROM (original_start - new_start)) / 86400.0;
      hours_saved := days_saved * 24;
    END IF;
    
    -- Apply 6-hour minimum threshold and movement caps
    IF best_gap IS NOT NULL 
       AND days_saved >= 0.25 
       AND days_saved <= v_stage_move_cap THEN
      
      -- CHANGE 4: Enhanced logging with capacity details
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % from % to % (saves %.2f days / %.1f hours; earliest_possible=%s; gap=[% → %]; finishing=%, cap=%d, lookback=%d)',
        gap_candidate.stage_name,
        original_start,
        new_start,
        days_saved,
      ...
      -- Create new gap-filled slot using new_start/new_end
      INSERT INTO stage_time_slots(
        production_stage_id,
        date,
        slot_start_time,
      ...
      -- Update job_stage_instances using new_start/new_end
      UPDATE job_stage_instances
      SET 
      ...
      VALUES (
        ...
        new_start,
        days_saved,
        (hours_saved * 60)::integer,
        'reschedule_all'
      );
      
      gap_filled_count := gap_filled_count + 1;
    ELSIF best_gap IS NOT NULL AND days_saved > v_stage_move_cap THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: would move %.1f days (exceeds cap %d)',
        gap_candidate.stage_name, days_saved, v_stage_move_cap;
    ELSIF best_gap IS NOT NULL AND days_saved < 0.25 THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: would only save %.2f days / %.1f hours (below 6-hour threshold)',
        gap_candidate.stage_name, days_saved, hours_saved;
    ELSIF best_gap IS NULL THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: no suitable gaps found (lookback=%d days, earliest_possible=%)',
        gap_candidate.stage_name, v_lookback_days, earliest_possible_start;
    END IF;
  END LOOP;
  
  RAISE NOTICE '✅ Capacity-aware gap-filling complete: % stages moved (tight packing enabled, 6-hour threshold)',
    gap_filled_count;
