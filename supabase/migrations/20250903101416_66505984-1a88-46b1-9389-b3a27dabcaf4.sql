-- Fix parallel stage scheduling by preserving existing queue tails in reschedule_all
...
BEGIN
  ...
  -- CRITICAL FIX: Initialize stage tails based on existing slots (like append_jobs does)
  -- This preserves existing queue tails for true parallel processing
  ...
  -- Ensure all relevant stages have entries (fallback to base_time for stages with no existing slots)
  INSERT INTO _stage_tails(stage_id, next_available_time)
