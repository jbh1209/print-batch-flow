-- Enhanced gap-filling: increase lookback windows and reduce future threshold
-- This allows finishing stages to look back up to 100% of their scheduling distance (max 30 days)
-- and other stages to look back 70% (up to 30 days), plus reduces future threshold to 1 day
...
BEGIN
  ...
  RAISE NOTICE '🔀 Starting Phase 2: Enhanced Gap-Filling (100%% finishing, 70%% others, 1-day threshold)';
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL 
      ...
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % from % to % (saves %.1f days, finishing=%, cap=%d, lookback=%d)',
        gap_candidate.stage_name,
        original_start,
        best_gap.gap_start,
        days_saved,
  ...
  RAISE NOTICE '✅ Enhanced gap-filling complete: % stages moved (finishing: 100%%, others: 70%%, threshold: 1 day)',
    gap_filled_count;
