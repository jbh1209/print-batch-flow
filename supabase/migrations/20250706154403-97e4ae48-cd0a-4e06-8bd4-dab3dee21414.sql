-- Comprehensive Multi-Stage Concurrent Workflow Fix
...
WHERE jsi.job_id = jcg.job_id
  AND jsi.job_table_name = jcg.job_table_name
  AND jsi.production_stage_id = ps.id
  AND ps.name ILIKE '%printing%' 
  AND ps.name NOT ILIKE '%queue%'
  AND ps.name NOT ILIKE '%laminating%';
...
-- 7. Fix any jobs that are currently stuck with printing stages
-- Activate both printing stages for jobs that have them pending after DTP/Proof completion
UPDATE public.job_stage_instances jsi1
SET 
  status = 'active',
  started_at = now(),
  started_by = auth.uid(),
  updated_at = now()
FROM public.job_stage_instances jsi2
JOIN public.production_stages ps1 ON jsi1.production_stage_id = ps1.id
JOIN public.production_stages ps2 ON jsi2.production_stage_id = ps2.id
WHERE jsi1.job_id = jsi2.job_id
  AND jsi1.job_table_name = jsi2.job_table_name
  AND jsi1.concurrent_stage_group_id = jsi2.concurrent_stage_group_id
  AND jsi1.concurrent_stage_group_id IS NOT NULL
  AND ps1.name ILIKE '%printing%' 
  AND ps1.name NOT ILIKE '%queue%'
  AND ps1.name NOT ILIKE '%laminating%'
  AND jsi1.status = 'pending'
  AND EXISTS (
    ...
    WHERE jsi_prev.job_id = jsi1.job_id
      AND jsi_prev.job_table_name = jsi1.job_table_name
      AND jsi_prev.stage_order < jsi1.stage_order
