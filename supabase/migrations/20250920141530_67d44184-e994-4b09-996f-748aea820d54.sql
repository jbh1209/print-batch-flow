-- Fix advance_job_stage to never auto-activate next stages
-- All stage completions should leave the next stage as "pending" for manual factory floor activation

CREATE OR REPLACE FUNCTION public.advance_job_stage(p_job_id uuid, p_job_table_name text, p_current_stage_id uuid, p_completed_by uuid DEFAULT auth.uid(), p_notes text DEFAULT NULL::text, p_actual_duration_minutes integer DEFAULT NULL::integer)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- CRITICAL FIX: Never auto-activate next stages - all completions require manual factory floor activation
  IF next_stage_id IS NOT NULL THEN
    RAISE NOTICE 'Stage % completed. Next stage remains pending for manual factory floor activation.', current_stage.stage_name;
