-- Updated scheduler function with proper sequential logic and "first available slot" scheduling
CREATE OR REPLACE FUNCTION public.calculate_daily_schedules(
  p_start_date date DEFAULT (CURRENT_DATE + '1 day'::interval), 
  p_end_date date DEFAULT (CURRENT_DATE + '15 days'::interval), 
...
BEGIN
  -- Calculate next business day (tomorrow if weekday, Monday if weekend)
  next_business_day := CURRENT_DATE + INTERVAL '1 day';
  WHILE EXTRACT(DOW FROM next_business_day) IN (0, 6) OR 
        public.is_public_holiday(next_business_day) 
  LOOP
    next_business_day := next_business_day + INTERVAL '1 day';
  END LOOP;
  
  -- Use calculated next business day as start date if not specified
  IF p_start_date <= CURRENT_DATE THEN
    p_start_date := next_business_day;
  END IF;
  ...
  INSERT INTO public.schedule_calculation_log (
    ...
    calc_run_id, p_calculation_type, 'Sequential scheduling from next business day at 8AM', start_time, auth.uid()
  );
  ...
  -- Phase 1: Schedule partially complete jobs first (priority)
  FOR job_record IN
    SELECT DISTINCT 
      jsi.job_id, 
      jsi.job_table_name,
      ...
      pj.created_at,
      MIN(CASE WHEN jsi.status = 'pending' THEN jsi.stage_order END) as next_stage_order
    FROM public.job_stage_instances jsi
    LEFT JOIN public.production_jobs pj ON jsi.job_id = pj.id AND jsi.job_table_name = 'production_jobs'
    WHERE jsi.status IN ('pending', 'completed')
      AND NOT EXISTS (
    ...
    HAVING COUNT(CASE WHEN jsi.status = 'completed' THEN 1 END) > 0 -- Has completed stages (partially complete)
    ORDER BY priority_score ASC, completed_stages DESC, pj.created_at ASC
    LIMIT 25
  LOOP
    jobs_processed_count := jobs_processed_count + 1;
    
    -- Schedule remaining stages for this job sequentially
    FOR stage_record IN
      SELECT 
    ...
    LOOP
      ...
      -- Find next available slot on current date or next business day
      WHILE processing_date <= p_end_date LOOP
        ...
        IF EXTRACT(DOW FROM processing_date) BETWEEN 1 AND 5 
          ...
          IF current_time_minutes + stage_record.duration_minutes <= (480 + 510) THEN -- End of business day (4:30 PM)
            -- Schedule the stage at current time slot
            ...
            -- Update daily schedule capacity
            ...
            -- Advance time: this stage completes and next stage can start immediately
            current_time_minutes := current_time_minutes + stage_record.duration_minutes;

            RAISE NOTICE 'Scheduled job % stage % (%s) on % at %s mins (partial job priority)', 
              job_record.job_id, stage_record.stage_name, stage_record.production_stage_id,
              processing_date, current_time_minutes;
  ...
  -- Phase 2: Schedule brand new jobs (no completed stages)
  FOR job_record IN
    SELECT DISTINCT 
      jsi.job_id, 
      jsi.job_table_name,
    ...
    GROUP BY jsi.job_id, jsi.job_table_name, pj.is_expedited, pj.created_at
    HAVING COUNT(CASE WHEN jsi.status = 'completed' THEN 1 END) = 0 -- No completed stages (brand new)
    ORDER BY priority_score ASC, pj.created_at ASC
    LIMIT 25
  LOOP
    jobs_processed_count := jobs_processed_count + 1;
    
    -- Schedule all stages for this new job sequentially
    FOR stage_record IN
      SELECT 
        jsi.id as stage_instance_id,
        jsi.production_stage_id,
    ...
    LOOP
      ...
      -- Find next available slot (same logic as partially complete jobs)
      WHILE processing_date <= p_end_date LOOP
        IF EXTRACT(DOW FROM processing_date) BETWEEN 1 AND 5 
           AND NOT public.is_public_holiday(processing_date) THEN
          
          IF current_time_minutes + stage_record.duration_minutes <= (480 + 510) THEN
            INSERT INTO public.job_schedule_assignments (
              job_id, job_table_name, production_stage_id, scheduled_date,
              queue_position, shift_number, estimated_duration_minutes, 
              priority_score, is_expedited, calculation_run_id, status
            ...
            RAISE NOTICE 'Scheduled job % stage % (%s) on % at %s mins (new job)', 
              job_record.job_id, stage_record.stage_name, stage_record.production_stage_id,
              processing_date, current_time_minutes;
  ...
  RETURN jsonb_build_object(
    ...
    'sequential_scheduling_info', jsonb_build_object(
      ...
      'logic', 'Sequential stage scheduling with first-available-slot and priority for partially complete jobs'
    ),
