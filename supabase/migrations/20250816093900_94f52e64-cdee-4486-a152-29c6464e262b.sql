-- STEP 3: Unified Schema Migration (Handle view dependencies with CASCADE)

-- Drop ALL validation and check functions first
...
DROP FUNCTION IF EXISTS run_scheduling_integrity_check() CASCADE;
...
-- Drop the old auto_scheduled columns WITH CASCADE to remove view dependencies
ALTER TABLE job_stage_instances 
DROP COLUMN IF EXISTS auto_scheduled_start_at CASCADE,
DROP COLUMN IF EXISTS auto_scheduled_end_at CASCADE,
DROP COLUMN IF EXISTS auto_scheduled_duration_minutes CASCADE;

-- Schema migration completed - dependent views dropped and will need to be recreated if needed