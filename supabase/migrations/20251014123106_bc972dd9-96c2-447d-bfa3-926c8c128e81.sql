-- Corrective migration: Restore working scheduler_append_jobs with gap-filling
-- This restores the working Phase 1 code (with atomicity and rollback) and adds Phase 2 gap-filling
...
DECLARE
  ...
  -- Atomicity tracking variables (RESTORED from working version)
  job_inserted_slot_ids uuid[];
  job_updated_stage_ids uuid[];
  job_failed boolean := false;
  
  -- Gap-filling variables (NEW for Phase 2)
...
BEGIN
  ...
  -- Initialize resource tails with ALL future slots (not just completed) - RESTORED
  INSERT INTO _stage_tails(stage_id, next_available_time)
  SELECT 
    production_stage_id, 
    COALESCE(MAX(slot_end_time), base_time)
  FROM stage_time_slots 
  WHERE slot_end_time >= base_time  -- RESTORED: All future slots
  ...
  -- PHASE 1: FIFO SCHEDULING WITH PARALLEL BARRIERS (RESTORED WORKING VERSION)
  ...
  LOOP
    -- RESTORED: Reset atomicity tracking for each job
    ...
    LOOP
      FOR r_stage IN
        ...
        ORDER BY jsi.id
      LOOP
        -- RESTORED: Proper error handling with EXIT instead of CONTINUE
        ...
        LOOP
          ...
          -- RESTORED: Track inserted slot IDs for rollback
        ...
        -- RESTORED: Proper error handling with EXIT
        ...
        -- RESTORED: Track updated stage IDs for rollback
      ...
      -- RESTORED: Check if stage group failed
      IF job_failed THEN
        EXIT;
      END IF;
    END LOOP;
    
    -- RESTORED: Complete atomicity rollback block
    IF job_failed THEN
  ...
  -- PHASE 2: GAP-FILLING WITH BOUNDED MULTI-PASS CONVERGENCE (NEW)
  ...
  FOR pass_iteration IN 1..3 LOOP
    ...
    FOR gap_candidate IN
      ...
      WHERE jsi.job_id = ANY(p_job_ids)  -- KEY: Only gap-fill appended jobs
