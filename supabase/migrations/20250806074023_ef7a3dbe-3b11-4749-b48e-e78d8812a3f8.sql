-- Fix the SQL query in calculate_daily_schedules to resolve DISTINCT with ORDER BY issue
  ...
  WHILE processing_date <= p_end_date LOOP
    ...
    IF EXTRACT(DOW FROM processing_date) BETWEEN 1 AND 5 
      ...
      LOOP
        ...
        -- Fixed: Remove DISTINCT and add jsi.created_at to SELECT to fix ORDER BY issue
        FOR job_record IN
