-- Fix convergence logic by ensuring proper stage ordering within each job
...
BEGIN
  ...
  LOOP
    ...
    -- CRITICAL FIX: Process stages in STRICT ORDER ensuring prerequisites come before convergence
    ...
    FOR r_stage IN
      ...
      ORDER BY 
        ...
        -- CRITICAL: Within same stage_order, process parts before 'both' stages
        CASE 
    ...
    LOOP
      ...
      IF r_stage.part_assignment = 'both' THEN
        ...
        BEGIN
          ...
          -- CRITICAL FIX: If parts aren't scheduled yet in current layer, use job barrier
          cover_end_time := COALESCE(
            (job_part_data ->> 'cover')::timestamptz, 
            job_current_barrier
          );
          text_end_time := COALESCE(
            (job_part_data ->> 'text')::timestamptz, 
            job_current_barrier
          );
          ...
          RAISE NOTICE 'FIXED CONVERGENCE: Stage % waits for cover=%s, text=%s, job_barrier=%s -> using convergence_barrier=%s',
            r_stage.stage_name, cover_end_time, text_end_time, job_current_barrier, convergence_barrier;
          
          -- Use the convergence barrier as the earliest start time
      ...
      -- ENHANCED: Update part-specific barriers for convergence tracking - IMMEDIATE UPDATE
      IF r_stage.part_assignment IN ('cover', 'text') THEN
        ...
        RAISE NOTICE 'PART TRACKING: IMMEDIATELY updated %s barrier to %s for job %s',
          r_stage.part_assignment, actual_end_time, r_job.job_id;
      END IF;

      -- For 'both' stages, don't add to layer_end_times until after convergence barrier is respected
      IF r_stage.part_assignment != 'both' OR 
         (r_stage.part_assignment = 'both' AND actual_start_time >= job_current_barrier) THEN
