-- Drop all existing versions of the function to eliminate conflicts
...
DROP FUNCTION IF EXISTS public.get_user_accessible_jobs_with_batch_allocation(uuid, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.get_user_accessible_jobs_with_batch_allocation(uuid, text, uuid, text);

-- Create the single, clean function with proper parameter types
