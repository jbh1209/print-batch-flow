-- Fix nuclear mode DELETE to clear ALL slots (including completed) to prevent constraint violations
-- CRITICAL: This migration ONLY changes the DELETE statement (lines 55-57)
-- CRITICAL: All other logic including FIFO enforcement and completed_barriers remains EXACTLY as-is

-- Problem: Nuclear mode was clearing only non-completed slots, causing duplicate key violations
-- when trying to re-insert slots at the same (production_stage_id, slot_start_time)
-- Solution: Clear ALL slots from base_time forward, since completed_barriers preserves the timing logic
...
BEGIN
  ...
  -- ========== FIXED: Nuclear mode clears ALL slots (including completed) ==========
  -- This prevents duplicate key violations on (production_stage_id, slot_start_time)
  -- Completed slot timings are preserved in completed_barriers logic below
  ...
  -- ========== PRESERVED: completed_barriers subquery fix (no changes) ==========
  completed_barriers := (
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- ========== PRESERVED: FIFO ENFORCEMENT comment (no changes) ==========
        -- FIFO ENFORCEMENT: Resource tail is NOT considered during Phase 1 scheduling
  ...
  -- ========== PRESERVED: Phase 2 Gap-Filling (no changes) ==========
