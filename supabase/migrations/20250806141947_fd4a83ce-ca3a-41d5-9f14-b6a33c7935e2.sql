-- Fix the ambiguous column reference error in calculate_daily_schedules function
...
BEGIN
  ...
  -- Clear existing future schedules only for the specific date range
  ...
  WHILE processing_date <= p_end_date LOOP
    ...
    IF EXTRACT(DOW FROM processing_date) BETWEEN 1 AND 5 
      ...
      LOOP
        ...
        FOR job_record IN
          SELECT jsi.job_id, jsi.job_table_name, jsi.created_at,
        ...
        LOOP
          ...
          IF current_allocation + job_record.duration_minutes <= daily_capacity_minutes THEN
            -- Schedule the job with ON CONFLICT handling
            INSERT INTO public.job_schedule_assignments (
              job_id, job_table_name, production_stage_id, scheduled_date,
              queue_position, shift_number, estimated_duration_minutes, priority_score, 
              is_expedited, calculation_run_id
            ) VALUES (
              job_record.job_id, job_record.job_table_name, stage_record.id, processing_date,
              queue_position, 1, job_record.duration_minutes, job_record.priority_score,
              job_record.is_expedited, calc_run_id
            ) ON CONFLICT (job_id, production_stage_id, scheduled_date, shift_number)
            DO UPDATE SET
  ...
  -- Log completion - Use renamed variables to avoid ambiguity
  ...
  RAISE NOTICE 'Calculation completed: % jobs processed, % stages affected', jobs_processed_count, stages_affected_count;
