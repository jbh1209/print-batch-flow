-- CRITICAL HOTFIX: Add missing proof approval filter to scheduler
-- This prevents unproven jobs like D426515 from being scheduled
...
BEGIN
  ...
  -- CRITICAL FIX: Process jobs in FIFO order by proof_approved_at
  -- ONLY SCHEDULE PROOF-APPROVED JOBS
  FOR r_job IN
    SELECT 
      pj.id as job_id,
    ...
    WHERE COALESCE(jsi.status, '') NOT IN ('completed', 'active')
      AND pj.proof_approved_at IS NOT NULL  -- CRITICAL: Only approved jobs
