...
SET 
  ...
  updated_at = now()
WHERE id IN (
  SELECT DISTINCT stage_instance_id 
  FROM public.stage_time_slots sts
  WHERE sts.slot_end_time > (date_trunc('day', sts.slot_end_time) + interval '16 hours 30 minutes')
     OR sts.slot_start_time < (date_trunc('day', sts.slot_start_time) + interval '8 hours')
);

-- Step 3: Create/replace enhanced time slot placement function with strict business hours
CREATE OR REPLACE FUNCTION public.place_duration_sql_business_hours(
...
BEGIN
  ...
  FOR day_counter IN 1..30 LOOP
    ...
    day_start := date_trunc('day', current_time) + working_start::interval;
    day_end := date_trunc('day', current_time) + working_end::interval;
    lunch_start := date_trunc('day', current_time) + lunch_start_time::interval;
    lunch_end := lunch_start + (lunch_duration || ' minutes')::interval;
    
    -- Skip weekends
    IF EXTRACT(dow FROM current_time) IN (0, 6) THEN
      current_time := date_trunc('day', current_time + interval '1 day') + working_start::interval;
      CONTINUE;
    END IF;
    
    -- Skip holidays
    IF EXISTS (SELECT 1 FROM public_holidays WHERE date = current_time::date AND COALESCE(is_active, true)) THEN
      current_time := date_trunc('day', current_time + interval '1 day') + working_start::interval;
      CONTINUE;
    END IF;
    
    -- Adjust current_time to start of working day if before hours
    ...
    IF current_time < lunch_start THEN
      available_minutes := EXTRACT(epoch FROM (lunch_start - current_time)) / 60;
      slot_minutes := LEAST(available_minutes, remaining_minutes);
      
      -- CRITICAL: Ensure slot doesn't exceed lunch time
      IF slot_minutes > 0 AND (current_time + (slot_minutes || ' minutes')::interval) <= lunch_start THEN
        RETURN QUERY SELECT current_time, current_time + (slot_minutes || ' minutes')::interval, slot_minutes;
    ...
    IF current_time >= lunch_end AND current_time < day_end AND remaining_minutes > 0 THEN
      available_minutes := EXTRACT(epoch FROM (day_end - current_time)) / 60;
      slot_minutes := LEAST(available_minutes, remaining_minutes);
      
      -- CRITICAL: Ensure slot doesn't exceed working day end
      IF slot_minutes > 0 AND (current_time + (slot_minutes || ' minutes')::interval) <= day_end THEN
        RETURN QUERY SELECT current_time, current_time + (slot_minutes || ' minutes')::interval, slot_minutes;
        remaining_minutes := remaining_minutes - slot_minutes;
      END IF;
    END IF;
...
BEGIN
  ...
  IF (NEW.slot_start_time::time >= '12:00:00' AND NEW.slot_start_time::time < '12:30:00') OR
     (NEW.slot_end_time::time > '12:00:00' AND NEW.slot_end_time::time <= '12:30:00') OR
     (NEW.slot_start_time::time < '12:00:00' AND NEW.slot_end_time::time > '12:30:00') THEN
