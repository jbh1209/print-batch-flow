-- Fix gap-filling to handle far-out stages like Final Trimming
-- Changes:
-- 1. Update gap-fill ordering to process earlier stages first (stage_order ASC)
-- 2. Replace fixed 5-day cap with dynamic caps (30 days for finishing, 21 for others)
-- 3. Add dynamic lookback calculation based on original schedule
-- 4. Enhanced logging for debugging

-- Update find_available_gaps to accept dynamic lookback parameter
...
-- Update scheduler_reschedule_all_parallel_aware with dynamic gap-filling
...
DECLARE
  ...
  -- UPDATED: Dynamic gap-filling constraints
...
BEGIN
  ...
  -- ========== PHASE 2: ENHANCED GAP-FILLING WITH DYNAMIC CONSTRAINTS ==========
  RAISE NOTICE '🔀 Starting Phase 2: Enhanced Gap-Filling (dynamic caps, earlier-stage-first ordering)';
  
  -- Calculate minimum future threshold: only fill gaps for stages at least 2 days out
  v_min_future_threshold := now() + interval '2 days';
  
  -- UPDATED: Process stages by stage_order ASCENDING first, then by scheduled_start_at DESC
  -- This ensures earlier stages get gap-filled first, enabling ripple-forward improvements
  ...
  LOOP
    ...
    -- UPDATED: Calculate dynamic movement cap based on stage type
    -- Finishing stages (trimming, packaging, etc.) can move up to 30 days
    -- Other stages can move up to 21 days
    IF gap_candidate.stage_name ILIKE ANY(ARRAY['%trimming%', '%packaging%', '%boxing%', '%dispatch%', '%shipping%']) THEN
      v_stage_move_cap := 30;
    ELSE
      v_stage_move_cap := 21;
    END IF;
    
    -- UPDATED: Calculate dynamic lookback based on how far out the stage is scheduled
    -- Stages further out get longer lookback windows (up to 30 days)
    v_days_until_original := EXTRACT(epoch FROM (original_start - now())) / 86400.0;
    v_lookback_days := LEAST(30, GREATEST(7, FLOOR(v_days_until_original * 0.5)::integer));
    
    RAISE NOTICE '🔀 Gap-fill candidate: stage % (order %): original=%s, earliest_possible=%s, move_cap=%d days, lookback=%d days',
    ...
    -- Find best gap with dynamic lookback
    ...
    -- UPDATED: Only move if found a gap AND it saves ≥1 day AND within dynamic movement cap
    IF best_gap IS NOT NULL 
       AND best_gap.days_earlier >= 1.0 
       AND best_gap.days_earlier <= v_stage_move_cap THEN
      
      days_saved := best_gap.days_earlier;
      
      RAISE NOTICE '🔀 GAP-FILLING: Moving stage % from %s to %s (saves %.1f days, cap=%d, lookback=%d)',
        gap_candidate.stage_name,
        original_start,
        best_gap.gap_start,
        days_saved,
    ...
    ELSIF best_gap IS NULL THEN
      RAISE NOTICE '🔀 SKIPPED gap-fill for stage %: no suitable gaps found (lookback=%d days)',
        gap_candidate.stage_name, v_lookback_days;
    END IF;
  END LOOP;
  
  RAISE NOTICE '✅ Enhanced gap-filling complete: % stages moved (dynamic caps: 30d finishing, 21d others; stage-order-first)',
    gap_filled_count;
