-- Fix scheduler time-awareness and on-hold/expired schedule handling
...
BEGIN
  ...
  -- PHASE 1 FIX: Use now() instead of tomorrow midnight for time-awareness
  ...
  RAISE NOTICE '🕐 Scheduler started at % (JNB: %)', now(), now() AT TIME ZONE 'Africa/Johannesburg';
  RAISE NOTICE '📅 Base time calculated as: % (JNB: %)', base_time, base_time AT TIME ZONE 'Africa/Johannesburg';
  ...
  -- PHASE 3 FIX: Clear expired schedules (scheduled_start_at in the past)
  ...
  SET 
    ...
    updated_at = now()
  WHERE status IN ('pending', 'on_hold', 'active')
    AND scheduled_start_at < now()
    AND schedule_status = 'scheduled';
  
  GET DIAGNOSTICS expired_count = ROW_COUNT;
  RAISE NOTICE '⏰ Cleared % stages with expired schedules (scheduled in the past)', expired_count;

  -- Clear schedule data ONLY for on_hold stages (they need fresh reschedule with remaining_minutes)
  -- Active stages keep their data as baseline but will be rescheduled
  UPDATE job_stage_instances 
  SET 
    scheduled_start_at = NULL,
    scheduled_end_at = NULL,
  ...
  GET DIAGNOSTICS on_hold_count = ROW_COUNT;
  RAISE NOTICE '🔄 Cleared % on_hold stages for rescheduling (will use remaining_minutes)', on_hold_count;
  ...
  -- Initialize stage availability for pending, active, and on_hold stages
  INSERT INTO _stage_tails(stage_id, next_available_time)
  SELECT DISTINCT jsi.production_stage_id, base_time
  FROM job_stage_instances jsi
  WHERE COALESCE(jsi.status, '') IN ('pending', 'active', 'on_hold')
  ON CONFLICT (stage_id) DO NOTHING;

  RAISE NOTICE 'Initialized % production stages', (SELECT COUNT(*) FROM _stage_tails);

  -- Process only approved jobs (excludes DTP/PROOF stages which happen before approval)
  FOR r_job IN
    SELECT 
      pj.id as job_id,
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        ELSIF r_stage.status = 'active' THEN
          RAISE NOTICE '🔄 Rescheduling ACTIVE stage % (% mins)', r_stage.stage_name, r_stage.duration_minutes;
        END IF;
