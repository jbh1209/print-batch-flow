...
-- Re-create the four-argument function with robust stage filter casting
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT
      job_id,
      ROUND((COUNT(*) FILTER (WHERE status IN ('completed', 'skipped'))::numeric / NULLIF(COUNT(*)::numeric, 0)) * 100) AS workflow_progress,
      COUNT(*) AS total_stages,
      COUNT(*) FILTER (WHERE status IN ('completed', 'skipped')) AS completed_stages
    FROM public.job_stage_instances
    WHERE job_id = pj.id AND job_table_name = 'production_jobs'
    GROUP BY job_id
