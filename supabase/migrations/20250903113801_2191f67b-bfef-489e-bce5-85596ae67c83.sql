-- Fix scheduler_reschedule_all_parallel_aware to properly initialize timing and clear scheduling data
...
BEGIN
  ...
  -- CRITICAL FIX: Clear all scheduling data from job_stage_instances to ensure clean slate
  ...
  RAISE NOTICE 'Cleared scheduling data from job_stage_instances';

  -- Initialize stage availability tracker
  PERFORM public.create_stage_availability_tracker();
  
  -- CRITICAL FIX: Since we're doing a full reschedule, initialize ALL stages to base_time
  -- This ensures all production queues start fresh from the next working day
  INSERT INTO _stage_tails(stage_id, next_available_time)
  SELECT DISTINCT jsi.production_stage_id, base_time
  FROM job_stage_instances jsi
  WHERE COALESCE(jsi.status, '') <> 'completed';

  RAISE NOTICE 'Initialized % production stages to base_time: %', (SELECT COUNT(*) FROM _stage_tails), base_time;
  ...
  LOOP
    ...
    RAISE NOTICE 'Processing job % (WO: %) with % stages - barriers initialized to %', 
      r_job.job_id, r_job.wo_no, r_job.total_stages, GREATEST(base_time, r_job.proof_approved_at);
    ...
    LOOP
      ...
      LOOP
        ...
        ELSE
          -- Independent stage or dependency group - use main barrier only
          -- CRITICAL FIX: Remove dependency group logic that relied on stale scheduled_end_at data
