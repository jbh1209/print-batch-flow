-- Fix gap-filling to respect business hours by using place_duration_sql for multi-day splits
-- This ensures gap-filled stages are split across days just like FIFO-scheduled stages
...
-- Update scheduler_append_jobs with proper gap-filling split logic
...
DECLARE
  ...
  days_saved numeric;
  gap_placement_result record;
  last_slot_end timestamptz;
BEGIN
  ...
  -- ========== PHASE 2: GAP-FILLING WITH PROPER MULTI-DAY SPLIT ==========
  RAISE NOTICE '🔀 Starting Phase 2: Gap-Filling with Multi-Day Split Support';
  ...
  LOOP
    ...
    IF best_gap IS NOT NULL AND best_gap.days_earlier >= 1.0 THEN
      ...
      -- CRITICAL FIX: Use place_duration_sql to properly split across days
      SELECT * INTO gap_placement_result
      ...
      IF NOT gap_placement_result.placement_success OR gap_placement_result.slots_created IS NULL THEN
        RAISE WARNING 'Gap-fill placement failed for stage % at %, skipping', 
          gap_candidate.stage_name, best_gap.gap_start;
        CONTINUE;
      END IF;
      
      -- Insert each slot (may be multiple if it spans days)
      FOR slot_record IN SELECT * FROM jsonb_array_elements(gap_placement_result.slots_created)
      LOOP
        INSERT INTO stage_time_slots(
          production_stage_id,
          date,
      ...
      -- Get the actual end time from the LAST slot created
      SELECT MAX((time_slot ->> 'end_time')::timestamptz)
      INTO last_slot_end
      FROM jsonb_array_elements(gap_placement_result.slots_created) time_slot;
      
      -- Update stage instance with actual placement times
      UPDATE job_stage_instances
      SET 
        scheduled_start_at = best_gap.gap_start,
        scheduled_end_at = last_slot_end,
        updated_at = now()
  ...
  RAISE NOTICE '✅ Gap-filling complete: % stages moved with proper multi-day splits', gap_filled_count;
...
-- Update scheduler_reschedule_all_parallel_aware with proper gap-filling split logic
    ...
    IF best_gap IS NOT NULL AND days_saved >= 0.25 THEN
      ...
      LOOP
        INSERT INTO stage_time_slots(
          ...
          (slot_record ->> 'end_time')::timestamptz,
          (slot_record ->> 'duration_minutes')::integer,
          gap_candidate.job_id, 'production_jobs',
