-- CRITICAL FIX: Refactor scheduler_append_jobs to use stage-wave scheduling
-- Current issue: Job D427310 Printing stage waits for earlier jobs to complete ALL stages
-- Fix: Process all jobs stage-by-stage (stage_order 1 for all jobs, then 2 for all jobs, etc.)
-- This enables parallel processing while respecting part_assignment dependencies
...
BEGIN
  ...
  -- PHASE 1: STAGE-WAVE SCHEDULING (not job-by-job)
  RAISE NOTICE '🌊 Phase 1: STAGE-WAVE Scheduling for % jobs', array_length(p_job_ids, 1);
  
  -- CRITICAL FIX: Outer loop iterates through stage_order waves
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- Calculate barrier-based earliest start
  ...
  -- PHASE 2: GAP-FILLING (unchanged from original)
...
COMMENT ON FUNCTION public.scheduler_append_jobs(uuid[], boolean) IS 
'STAGE-WAVE scheduler: Processes all jobs stage-by-stage (not job-by-job) to enable parallel processing. For stage_order 1, schedules all jobs; then stage_order 2 for all jobs, etc. Respects part_assignment dependencies (cover/text/both convergence).';
