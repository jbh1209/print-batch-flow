-- Fix #1: job_earliest_start NULL regression in scheduler_append_jobs
-- Fix #2: Hardcoded 08:00 in place_duration_sql  
-- Fix #3: Missing data integrity validations
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- FIX #1: Respect existing waves with proper NULL handling
      ...
      -- Use wave_barrier only if it's not NULL and greater than current barrier
      IF wave_barrier IS NOT NULL AND wave_barrier > job_earliest_start THEN
        job_earliest_start := wave_barrier;
      END IF;
      ...
      LOOP
        ...
        IF r_stage.stage_instance_id IS NULL THEN
          RAISE WARNING 'Invalid stage instance ID found, skipping';
          CONTINUE;
        END IF;

        IF r_stage.production_stage_id IS NULL THEN
          RAISE WARNING 'Invalid production stage ID for stage instance %, skipping', r_stage.stage_instance_id;
          CONTINUE;
        END IF;
        ...
        IF placement_result.placement_success AND placement_result.slots_created IS NOT NULL THEN
          ...
          LOOP
            -- FIX #3: Enhanced data integrity checks for JSONB extractions
            ...
            IF (slot_record ->> 'start_time')::timestamptz >= (slot_record ->> 'end_time')::timestamptz THEN
              RAISE WARNING 'Invalid time ordering in slot for stage %: start >= end', r_stage.stage_instance_id;
              CONTINUE;
            END IF;

            -- Validate duration consistency
            IF (slot_record ->> 'duration_minutes')::integer <= 0 THEN
              RAISE WARNING 'Invalid duration in slot for stage %: % minutes', r_stage.stage_instance_id, (slot_record ->> 'duration_minutes')::integer;
              CONTINUE;
            END IF;
          ...
          -- FIX #3: Validate stage_end_time
          IF stage_end_time IS NULL THEN
            RAISE WARNING 'Failed to calculate stage end time for stage %', r_stage.stage_instance_id;
            CONTINUE;
          END IF;
...
-- FIX #2: Remove hardcoded 08:00 from place_duration_sql
...
BEGIN
  ...
  WHILE remaining_minutes > 0 AND day_count < p_max_days LOOP
    ...
    IF NOT public.is_working_day(current_day) THEN
      ...
      -- FIX #2: Use next_working_start instead of hardcoded 08:00
