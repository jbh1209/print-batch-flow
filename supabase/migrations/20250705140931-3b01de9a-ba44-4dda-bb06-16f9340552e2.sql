-- ARCHITECTURAL FIX: Resolve Batch Job References Foreign Key Issues
-- Step 1: Fix the trigger function to properly match jobs by job_number

-- Drop existing problematic function and recreate with proper logic
...
BEGIN
  ...
  IF NEW.batch_id IS NOT NULL AND (OLD IS NULL OR OLD.batch_id IS NULL OR OLD.batch_id != NEW.batch_id) THEN
    ...
    -- Get the job number to match (handle different column names)
    job_number_to_match := COALESCE(NEW.job_number, NEW.wo_no);
    
    IF job_number_to_match IS NULL THEN
    ...
    ELSE
      RAISE WARNING 'No matching production job found for % job % (ID: %) - batch reference not created', 
...
-- Step 2: Recreate triggers for all job tables with improved function
DO $$
DECLARE
  job_table text;
...
-- Step 3: Add index for faster job_number lookups
CREATE INDEX IF NOT EXISTS idx_production_jobs_wo_no ON public.production_jobs(wo_no);

-- Step 4: Create validation function to check job matching
  ...
  FOREACH job_table IN ARRAY job_tables LOOP
    RETURN QUERY EXECUTE format('
      SELECT 
        ...
        j.id::uuid as job_id,
        COALESCE(j.job_number, j.wo_no)::text as job_number,
        (pj.id IS NOT NULL)::boolean as has_production_job,
        pj.id::uuid as production_job_id,
        j.batch_id::uuid as batch_id
      FROM %I j
      LEFT JOIN public.production_jobs pj ON pj.wo_no = COALESCE(j.job_number, j.wo_no)
...
-- Step 5: Create repair function for existing broken references
  ...
  FOREACH job_table IN ARRAY job_tables LOOP
    FOR job_record IN EXECUTE format('
      SELECT 
        j.id,
        j.batch_id,
        COALESCE(j.job_number, j.wo_no) as job_number
      FROM %I j
      WHERE j.batch_id IS NOT NULL
        AND NOT EXISTS (
