-- =====================================================
-- CLEANUP: Remove orphaned scheduled times from DTP/PROOF stages
-- =====================================================
...
WHERE jsi.production_stage_id = ps.id
  AND ps.name IN ('DTP', 'PROOF');

-- =====================================================
-- ENHANCEMENT: Add explicit DTP/PROOF exclusion to scheduler
-- =====================================================
    ...
    LOOP
      ...
      FOR r_stage IN
        ...
        WHERE jsi.id = ANY(r_stage_group.stage_instance_ids)
          AND ps.name NOT IN ('DTP', 'PROOF')  -- ← EXPLICIT EXCLUSION ADDED
        ORDER BY jsi.id
      LOOP
