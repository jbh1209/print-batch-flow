  ...
  for (const job of jobs) {
    ...
    for (const ord of orders) {
      for (const st of stages.filter((s: any) => (s.stage_order ?? 9999) === ord)) {
        ...
        // compute earliest start respecting approvals, deps, and resource free time
        ...
        // ---------------- minutes source (FIX) ----------------
        // Prefer explicit per-stage minutes, then estimates(+setup),
        // then fall back to job-level minutes if present.
        let minsRaw = 0;

        if (st.scheduled_minutes && st.scheduled_minutes > 0) {
          minsRaw = st.scheduled_minutes;
        } else {
          const est = (st.estimated_minutes ?? 0) + (st.setup_minutes ?? 0);
          if (est > 0) {
            minsRaw = est;
          } else if (st.run_minutes && st.run_minutes > 0) {
            minsRaw = st.run_minutes;
          } else if (job.estimated_run_minutes && job.estimated_run_minutes > 0) {
            minsRaw = job.estimated_run_minutes;
          } else if (job.total_minutes && job.total_minutes > 0) {
            minsRaw = job.total_minutes;
          }
        }

        const mins = Math.max(1, Math.round(minsRaw));
        // ------------------------------------------------------
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (nuclear) {
      ...
      if (commit) {
        if (wipeAll) {
          // Try with wipe_all; if DB rejects, fall back to from_date only
