...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (nuclear) {
      ...
      if (commit) {
        // Try with wipe_all; if DB doesn't accept it, fall back to from_date only.
        let triedWithWipe = false;
        if (wipeAll) {
          triedWithWipe = true;
          const { error } = await supabase.rpc("unschedule_auto_stages",
            { from_date: unscheduledFromDate, wipe_all: true });
          if (error) {
            // fall back
            const { error: e2 } = await supabase.rpc("unschedule_auto_stages",
              { from_date: unscheduledFromDate });
            if (e2) throw new Error("unschedule_auto_stages failed (wipe_all + fallback): " +
