...
// Accepts boolean true/false, "true"/"false", 1/"1"/0/"0"
...
function nextWorkingStart(input: SchedulerInput, from: Date) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // FIX: accept booleans or strings
    ...
    const startFrom: Date | null = startFromStr ? new Date(startFromStr) : null;
    ...
    // 2) Compute base start for nuclear runs + optional wipe
    let baseStart: Date | undefined;
    let unscheduledFromDate: string | undefined;

    if (nuclear) {
      const seed = startFrom ? startFrom : addMin(new Date(), 24*60); // tomorrow if not provided
      ...
      if (commit) {
        const { error: clearErr } = await supabase.rpc("unschedule_auto_stages",
          { from_date: unscheduledFromDate, wipe_all: wipeAll });
    ...
    if (commit && updates.length) {
      ...
      // Mirror into board table used by Schedule page
