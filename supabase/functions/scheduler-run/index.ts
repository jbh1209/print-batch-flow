...
interface JobRow {
  ...
  due_date: string | null; proof_approved_at: string | null;
  estimated_run_minutes: number; stages: StageRow[];
}
interface StageRow {
  ...
  scheduled_minutes: number | null; schedule_status: string | null;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const proposed    = toBool(qp("proposed",     false), false);
    const onlyIfUnset = toBool(qp("onlyIfUnset",  false), false);
    const nuclear     = toBool(qp("nuclear",      false), false);
    const wipeAll     = toBool(qp("wipeAll",      false), false);
    const startFrom   = String(qp("startFrom", "") || "");
    ...
    // 2) Nuclear: compute base start and clear
    ...
    if (commit && updates.length) {
      ...
      // IMPORTANT: mirror to board table
