...
// ---- Time helpers (Africa/Johannesburg by default) ----
const LOCAL_TZ = "Africa/Johannesburg";          // change if needed
const MS = 60_000;
const addMin = (d: Date, m: number) => new Date(d.getTime() + m * MS);

// Clamp a Date into a specific IANA timezone by rebuilding Y-M-D h:m:s
const toLocal = (d: Date, tz = LOCAL_TZ) =>
  new Date(new Intl.DateTimeFormat("en-CA", {
    timeZone: tz, hour12: false, year: "numeric", month: "2-digit", day: "2-digit",
    hour: "2-digit", minute: "2-digit", second: "2-digit"
  }).format(new Date(d)).replace(", ", "T") + "Z");

const dayStart = (d: Date, tz = LOCAL_TZ) => {
  const l = toLocal(d, tz);
  return toLocal(new Date(`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}-${String(l.getDate()).padStart(2,"0")}T00:00:00`), tz);
};
...
interface StageRow {
  ...
  scheduled_start_at: string | null; scheduled_end_at: string | null; scheduled_minutes: number | null;
  schedule_status: string | null; production_stage_id: UUID;
}
...
function dailyWindows(input: SchedulerInput, day: Date): Interval[] {
  ...
  for (const s of todays) {
    ...
    const start = toLocal(new Date(day.getFullYear(), day.getMonth(), day.getDate(), st.h, st.m, +st.s));
    const end   = toLocal(new Date(day.getFullYear(), day.getMonth(), day.getDate(), et.h, et.m, +et.s));
    if (end <= start) continue;

    // start with the whole shift, then cut out breaks
    let segs: Interval[] = [{ start, end }];

    for (const br of (input.meta.breaks ?? [])) {
      const bt = parseClock(br.start_time);
      const b0 = toLocal(new Date(day.getFullYear(), day.getMonth(), day.getDate(), bt.h, bt.m, +bt.s));
...
function* iterWindows(input: SchedulerInput, from: Date, horizonDays = 365): Generator<Interval> {
  for (let i = 0; i < horizonDays; i++) {
    const base = dayStart(from, LOCAL_TZ);
    const day  = addMin(base, i * 24 * 60);

    if (isHoliday(input.holidays, day)) continue;

    for (const w of dailyWindows(input, day)) {
      if (w.end <= from) continue;                          // never place in the past
...
// place a duration contiguously across windows
...
function planSchedule(input: SchedulerInput, baseStart?: Date) {
  const jobs = input.jobs
    ...
    .map(j => ({ ...j, approvedAt: toLocal(new Date(j.proof_approved_at!)) }))
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const wipeAll     = qp("wipeAll","false")==="true";     // optional
    ...
    // 2) Compute base start & clear when nuclear
    let baseStart: Date | undefined;
    if (nuclear) {
      // “today” in local TZ, then first working window ON/AFTER that
      const seed = startFrom ? toLocal(new Date(startFrom + "T00:00:00")) : toLocal(new Date());
    ...
    // 4) Apply and MIRROR to stage_time_slots
    ...
    if (commit && updates.length) {
      ...
      // IMPORTANT: mirror so the Schedule Board (which reads stage_time_slots) can render
