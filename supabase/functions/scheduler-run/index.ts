...
interface StageRow {
  ...
  part_name: string | null; part_type: string | null; stage_name: string; stage_group_id: UUID | null;
  stage_group_name: string | null; category_id: UUID | null; stage_order: number | null;
  ...
  previous_stage_id: UUID | null; dependency_group: string | null; production_stage_id: UUID;
}
...
function isHoliday(holidays: SchedulerInput['holidays'], day: Date){
  ...
  const key = `${y}-${m}-${d}`; return holidays.some(h => h.date.startsWith(key));
}
...
function* iterateWorkingWindows(input: SchedulerInput, from: Date, horizonDays=90): Generator<Interval> {
  ...
  for (let i=0;i<horizonDays;i++){
    ...
    for (const w of wins) {
      ...
      yield { start: new Date(Math.max(w.start.getTime(), from.getTime())), end: w.end };
    }
  }
}
...
function planSchedule(input: SchedulerInput): ScheduleResult {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (exportErr) {
      return new Response(JSON.stringify({ error: "export_scheduler_input failed", detail: exportErr }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    ...
    if (commit && updates.length) {
      ...
      if (error) {
        return new Response(JSON.stringify({ error: "apply_stage_updates_safe failed", detail: error }), {
          status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
      applied = data;
    ...
    return new Response(JSON.stringify({ ok: true, scheduled: updates.length, applied, updates }), {
