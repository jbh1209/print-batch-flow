...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Read flags (camelCase in the request body)
    const nuclear     = qp("nuclear","false")==="true";
    const startFrom   = qp("startFrom","");     // yyyy-mm-dd
    const wipeAll     = qp("wipeAll","false")==="true";

    // Force writer flags ON (to eliminate param casing mistakes)
    const commit      = true;
    const proposed    = false;
    const onlyIfUnset = false;

    console.log("scheduler-run flags:", { commit, proposed, onlyIfUnset, nuclear, startFrom, wipeAll });
    ...
    if (nuclear) {
      ...
      console.log("computed baseStart:", baseStart?.toISOString());
